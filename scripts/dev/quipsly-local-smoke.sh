...
is_local_target_url() {
  [[ "${TARGET_URL}" == http://127.0.0.1:* || "${TARGET_URL}" == http://localhost:* ]]
}
...
if is_local_target_url; then
  check_contains "/account/switch" "Quipsly profile vault"
  check_contains "/account/switch" "No current session"
  check_contains "/admin/users" "User + Invite Console"
  check_contains "/admin/users" "Invite someone by email"
else
