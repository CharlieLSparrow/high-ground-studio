...
SCHEDULER_JOB="${SCHEDULER_JOB:-quipsly-transactional-email}"
SCHEDULE="${SCHEDULE:-*/2 * * * *}"
SCHEDULER_SERVICE_ACCOUNT="${SCHEDULER_SERVICE_ACCOUNT:-quipsly-transactional-email@${PROJECT_ID}.iam.gserviceaccount.com}"
...
if [[ ! "${PROJECT_ID}" =~ ^[a-z][a-z0-9-]{4,62}$ ]] \
  ...
  echo "Transactional email scheduler project, region, service, job, or identity is unsafe." >&2
  exit 2
fi
...
target_uri="${service_url}/api/cron/transactional-email"
...
if ! gcloud iam service-accounts describe "${SCHEDULER_SERVICE_ACCOUNT}" \
  ...
  gcloud iam service-accounts create "${service_account_id}" \
    --project="${PROJECT_ID}" \
    --display-name="Quipsly transactional email scheduler" \
    --quiet
fi
...
if (JSON.stringify(job).includes("QUIPSLY_TRANSACTIONAL_EMAIL_SECRET")) failures.push("embedded secret");
if (failures.length) {
  process.stderr.write(`Transactional email scheduler readback failed: ${failures.join(", ")}\n`);
  process.exit(1);
}
