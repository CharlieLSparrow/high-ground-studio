...
const script = await readFile(
  new URL("./quipsly-media-processor-access.sh", import.meta.url),
  "utf8",
);
...
test("media processor access declares every worker control and derivative boundary", () => {
  for (const prefix of [
    ...
    "media-vault/control/session-audio-audition/",
    "media-vault/control/session-recording-share/",
    "media-vault/proxy/",
...
test("mastering output is create-and-read only while control records stay updateable", () => {
  assert.match(
    script,
    /"\$\{mastering_folder\}"[\s\S]+roles\/storage\.objectCreator/,
  );
  assert.match(
    script,
    /"\$\{mastering_folder\}"[\s\S]+roles\/storage\.objectViewer/,
  );
  assert.doesNotMatch(
    script,
    /"\$\{mastering_folder\}"[\s\S]{0,140}roles\/storage\.objectUser/,
  );
  assert.match(
    script,
    /"\$\{spectral_folder\}"[\s\S]+roles\/storage\.objectCreator/,
  );
  assert.match(
    script,
    /"\$\{spectral_folder\}"[\s\S]+roles\/storage\.objectViewer/,
  );
  assert.doesNotMatch(
    script,
    /"\$\{spectral_folder\}"[\s\S]{0,140}roles\/storage\.objectUser/,
  );
  assert.match(
    script,
    /"\$\{session_recording_share_folder\}"[\s\S]+roles\/storage\.objectCreator/,
  );
  assert.match(
    script,
    /"\$\{session_recording_share_folder\}"[\s\S]+roles\/storage\.objectViewer/,
  );
  assert.doesNotMatch(
    script,
...
test("access activation never grants job execution overrides", () => {
  ...
  assert.match(
    script,
    /Processor access is request-driven; Cloud Scheduler remains disabled by contract/,
  );
