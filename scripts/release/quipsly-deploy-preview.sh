...
TRANSCRIPT_FOLLOW_THROUGH_SCHEDULER_SERVICE_ACCOUNT="${TRANSCRIPT_FOLLOW_THROUGH_SCHEDULER_SERVICE_ACCOUNT:-quipsly-transcript-follow-through@${PROJECT_ID}.iam.gserviceaccount.com}"
...
if [[ ! "${QUIPSLY_GA_MEASUREMENT_ID}" =~ ^G-[A-Z0-9]+$ ]]; then
  echo "QUIPSLY_GA_MEASUREMENT_ID must be a GA4 measurement ID such as G-XXXXXXXXXX." >&2
  exit 2
fi
...
if [[ "${ENABLE_TRANSCRIPT_WORKER}" == "1" ]] && {
  [[ ! "${TRANSCRIPT_WORKER_PROJECT_ID}" =~ ^[a-z][a-z0-9-]{4,62}$ ]] \
    ...
    || [[ ! "${TRANSCRIPT_FOLLOW_THROUGH_SCHEDULER_SERVICE_ACCOUNT}" =~ ^[a-z0-9][a-z0-9-]{4,28}@[a-z][a-z0-9-]{4,62}\.iam\.gserviceaccount\.com$ ]] \
...
transactional_email_worker_env_vars=""
...
if [[ "${ENABLE_SESSION_INVITATION_EMAIL}" == "1" ]]; then
  ...
  transactional_email_audience="$(
    gcloud run services describe "${SERVICE_NAME}" \
      --project="${PROJECT_ID}" \
      --region="${REGION}" \
      --format='value(status.url)'
  )"
  if [[ ! "${transactional_email_audience}" =~ ^https://[a-z0-9-]+-[a-z0-9]+\.[a-z0-9-]+\.run\.app$ ]]; then
    echo "Could not resolve a safe Cloud Run audience for transactional email maintenance." >&2
    exit 2
  fi
  transactional_email_worker_env_vars=",QUIPSLY_TRANSACTIONAL_EMAIL_SERVICE_ACCOUNT=${TRANSACTIONAL_EMAIL_SCHEDULER_SERVICE_ACCOUNT},QUIPSLY_TRANSACTIONAL_EMAIL_AUDIENCE=${transactional_email_audience}"
...
NODE
  ...
  transcript_follow_through_audience="$(
    gcloud run services describe "${SERVICE_NAME}" \
      --project="${PROJECT_ID}" \
      --region="${REGION}" \
      --format='value(status.url)'
  )"
  if [[ ! "${transcript_follow_through_audience}" =~ ^https://[a-z0-9-]+-[a-z0-9]+\.[a-z0-9-]+\.run\.app$ ]]; then
    echo "Could not resolve a safe Cloud Run audience for transcript follow-through maintenance." >&2
    exit 2
  fi
  transcript_follow_through_env_vars=",CAPTURE_TRANSCRIPT_FOLLOW_THROUGH_SERVICE_ACCOUNT=${TRANSCRIPT_FOLLOW_THROUGH_SCHEDULER_SERVICE_ACCOUNT},CAPTURE_TRANSCRIPT_FOLLOW_THROUGH_AUDIENCE=${transcript_follow_through_audience}"
