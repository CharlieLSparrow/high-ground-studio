...
requireIncludes(episodeChatText, "CaptureSessionChatOpenButton", "exact-call Session chat is reachable beside the primary recorder");
requireIncludes(episodeChatText, "QuipslyCapture/SessionChat", "Session chat uses a distinct protected cache namespace");
requireIncludes(episodeChatText, "payload.thread?.key == context.threadKey", "native chat accepts only the exact requested durable thread");
requireIncludes(episodeChatText, 'requestBody[scope == .episode ? "episodeSlug" : "threadKey"]', "native writes preserve the exact Episode or Session scope");
requireIncludes(episodeChatText, "Posts stay with this exact call.", "Session chat states its exact-call collaboration boundary");
