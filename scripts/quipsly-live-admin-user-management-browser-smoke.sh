...
DATABASE_SECRET="${QUIPSLY_ADMIN_BROWSER_SMOKE_DATABASE_SECRET:-studio-database-url}"
CLOUD_SQL_INSTANCE="${QUIPSLY_CLOUD_SQL_INSTANCE:-high-ground-odyssey:us-central1:studio-postgres}"
PROXY_PORT="${QUIPSLY_SMOKE_CLOUD_SQL_PROXY_PORT:-15442}"
BASE_URL="${QUIPSLY_ADMIN_BROWSER_SMOKE_BASE_URL:-https://nest.quipsly.com}"

echo "Running Quipsly live admin user-management browser smoke against ${BASE_URL}"
...
if printf '%s' "${DB_URL}" | rg -q '/cloudsql/'; then
  ...
  "${PROXY_BIN}" \
    ...
    --port "${PROXY_PORT}" \
    "${CLOUD_SQL_INSTANCE}" \
    >/tmp/quipsly-admin-browser-smoke-proxy.log 2>&1 &
  PROXY_PID=$!
  ...
  DATABASE_URL="${DB_URL}" \
    ...
    QUIPSLY_ADMIN_BROWSER_SMOKE_BASE_URL="${BASE_URL}" \
    node scripts/quipsly-admin-user-management-browser-smoke.mjs
else
  DATABASE_URL="${DB_URL}" \
    QUIPSLY_ADMIN_BROWSER_SMOKE_BASE_URL="${BASE_URL}" \
    node scripts/quipsly-admin-user-management-browser-smoke.mjs
