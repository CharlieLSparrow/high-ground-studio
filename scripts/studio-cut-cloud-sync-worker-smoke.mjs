...
const publishPackageDir = getArgValue("--publish-package-dir");
const publishPackageScenario = getArgValue("--publish-package-scenario") ?? "short";
const onlyScenario = getArgValue("--only-scenario");
...
async function writePublishPackage(outputDir, result) {
  const resolvedOutputDir = path.resolve(repoRoot, outputDir);
  const files = {
    manifest: "episode-manifest.synthetic.json",
    sourceMonitorProxy: "source-monitor-proxy.synthetic.mp4",
  ...
  await copyFile(
    ...
    path.join(resolvedOutputDir, files.manifest),
  );
  await copyFile(
    result.packageFiles.sourceMonitorProxyPath,
    path.join(resolvedOutputDir, files.sourceMonitorProxy),
  );
  await copyFile(
    result.packageFiles.syncMapPath,
    path.join(resolvedOutputDir, files.syncMap),
  );
  await copyFile(
    result.packageFiles.reportPath,
    path.join(resolvedOutputDir, files.syncReport),
  );
  await copyFile(
  ...
  await writeJson(path.join(resolvedOutputDir, files.summary), {
    ...
    purpose: "Synthetic Studio Cut generated-package publish smoke.",
    projectId: result.projectId,
    branchId: result.branchId,
    title: result.title,
    sourceMonitorDurationMs: result.sourceMonitorDurationMs,
    sourceMonitorResolution: result.sourceMonitorResolution,
    files,
    roomUrl: `https://high-ground-odyssey.web.app/?projectId=${result.projectId}&branchId=${result.branchId}`,
  });

  await writeFile(
    path.join(resolvedOutputDir, files.readme),
    `# Synthetic Studio Cut Rescue Sync Package

This directory is generated by \`pnpm studio-cut:rescue-sync-package\`.
It is ignored by git and contains synthetic media only.

Use it to test the live Studio Cut generated-package publish flow without
private footage, local paths, credentials, or real episode metadata.

## Package

- Project: \`${result.projectId}\`
- Branch: \`${result.branchId}\`
- Room URL: \`https://high-ground-odyssey.web.app/?projectId=${result.projectId}&branchId=${result.branchId}\`
...
3. Switch Collaboration Mode to:
   - projectId: \`${result.projectId}\`
   - branchId: \`${result.branchId}\`
4. In \`Publish Rescue Sync Package\`, select:
   - Manifest: \`${files.manifest}\`
   - Source-monitor proxy: \`${files.sourceMonitorProxy}\`
   - Sync Map: \`${files.syncMap}\`
   - Sync report: \`${files.syncReport}\`
5. Click \`Publish Generated Package\`.
6. Open or copy the room link and confirm Shared Room Diagnostics show:
...
async function main() {
  ...
  const activeScenarios = onlyScenario
    ? scenarios.filter((scenario) => scenario.name === onlyScenario)
    : scenarios;

  if (activeScenarios.length === 0) {
    throw new Error(`Unknown scenario for --only-scenario: ${onlyScenario}`);
  }

  if (
    publishPackageDir &&
    !activeScenarios.some((scenario) => scenario.name === publishPackageScenario)
  ) {
    throw new Error(
      `--publish-package-scenario ${publishPackageScenario} must be one of the active scenarios`,
    );
  }
  ...
  try {
    ...
    if (publishPackageDir) {
      const packageResult = results.find(
        (result) => result.name === publishPackageScenario,
      );

      if (!packageResult) {
        throw new Error(`No result for package scenario ${publishPackageScenario}`);
      }

      const outputDir = await writePublishPackage(publishPackageDir, packageResult);
      console.log(`Wrote synthetic Rescue Sync publish package: ${outputDir}`);
    }
