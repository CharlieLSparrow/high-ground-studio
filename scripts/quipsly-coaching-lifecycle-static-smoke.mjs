...
const paths = {
  ...
  transcriptRunner: "apps/quipsly/src/lib/server/capture-transcripts.ts",
...
const checks = [
  ...
  check(
    includesAll(texts.transcriptRunner || "", [
    ...
    "Transcript runner turns verified media into append-only inspectable segments, versions reruns, and marks held/failed states calmly.",
  ),
  check(
    includesAll(texts.packetRouteAdapter || "", [
      ...
      "Only ACCEPT materializes one unassigned Quipsly ActionItem.",
    ]),
