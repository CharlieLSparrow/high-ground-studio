...
function packet(baseURL) {
  return {
    ...
    identities: {
      coach: { role: "coach", email: "acceptance-coach-a1b2c3d4@dev.test", displayName: "Coach", firebaseUid: "firebase_coach_123", userId: "user_coach_123" },
      client: { role: "client", email: "acceptance-client-a1b2c3d4@dev.test", displayName: "Client", firebaseUid: "firebase_client_123", userId: "user_client_123" },
    },
    boundaries: { passwordsStoredInMacOSKeychain: true, passwordsWrittenToArtifact: false },
  };
}

test("returns null when fresh continuation is not requested", async () => {
  assert.equal(await loadFreshCoachingAcceptanceContext({ baseURL: "http://127.0.0.1:3012", env: {} }), null);
});

test("loads a private fresh context without inventing fixture authority", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "quipsly-fresh-context-"));
  const contextPath = path.join(directory, "context.json");
  await writeFile(contextPath, JSON.stringify(packet("http://127.0.0.1:3012")), { mode: 0o600 });
  ...
  assert.equal(loaded.identities.coach.email, "acceptance-coach-a1b2c3d4@dev.test");
});
...
test("rejects a context exposed to other local users", async () => {
  ...
  await assert.rejects(
    loadFreshCoachingAcceptanceContext({ baseURL: "http://127.0.0.1:3012", env: { QUIPSLY_COACHING_ACCEPTANCE_CONTEXT: contextPath } }),
