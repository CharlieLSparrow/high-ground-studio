...
import {
  FfmpegInterruptionRepairEngine,
} from "../apps/quipsly-media-processor/src/interruption-repair-ffmpeg.ts";

if (process.env.QUIPSLY_LOCAL_INTERRUPTION_REPAIR_OPERATION !== "1") {
  throw new Error("Set QUIPSLY_LOCAL_INTERRUPTION_REPAIR_OPERATION=1 to authorize local retained repair artifacts.");
}
...
const { promoteRepairedMobileCaptureDatabaseEvidence } = await import(
...
try {
  ...
  const job = jobs.find((candidate) => candidate.inputJson?.source?.storageBackend === "local-development");
  assert(job, "No queued local interruption repair job is available.");
  const input = job.inputJson;
  const source = input.source;
  const capture = input.capture;
  assert(capture && typeof capture === "object", "Local repair job predates the complete promotion binding; rerun recorder crash recovery first.");
  ...
  assert(boundaryIndex > 0, "Local repair source is outside the Capture vault object boundary.");
  const objectsRoot = originalPath.slice(0, boundaryIndex + objectBoundary.length - 1);
  ...
  assert(relative && !relative.startsWith("..") && !path.isAbsolute(relative), "Local repair target escaped the Capture vault.");
  await mkdir(path.dirname(repairedPath), { recursive: true });
  const originalHashBefore = await hashFile(originalPath);
  assert(originalHashBefore.sizeBytes === source.sizeBytes, "Local repair source size changed before processing.");
  assert(originalHashBefore.sha256 === source.sha256, "Local repair source SHA-256 changed before processing.");

  const repaired = await new FfmpegInterruptionRepairEngine().repair(originalPath, repairedPath);
  ...
  const [completedJob, recording, receipt] = await Promise.all([
    ...
    prisma.mobileCaptureFinalizationReceipt.findUnique({ where: { uploadSessionId: input.uploadSessionId } }),
  ]);
  const originalHashAfter = await hashFile(originalPath);
  assert(completedJob?.status === "completed", "Repair workflow did not complete.");
  assert(recording?.localManifestJson?.interruptionRepair?.status === "verified", "RecordingAsset repair did not become verified.");
  assert(receipt?.sourceId && receipt?.mediaAssetId, "Repaired source was not promoted into Studio.");
  assert(originalHashAfter.sha256 === originalHashBefore.sha256, "Repair mutated the immutable original.");
  assert(originalHashAfter.sizeBytes === originalHashBefore.sizeBytes, "Repair changed the immutable original size.");
  assert((await stat(repairedPath)).size === repaired.sizeBytes, "Promoted repair derivative changed after verification.");
...
async function hashFile(filePath) {
  ...
  try {
    ...
    while (true) {
      const { bytesRead } = await handle.read(buffer, 0, buffer.length, sizeBytes);
