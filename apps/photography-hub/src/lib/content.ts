...
export function markdownToHtml(markdown: string, accentColor: "teal" | "amber" | "rose" = "amber"): string {
  let html = markdown
  ...
  html = html.replace(/^# (.*$)/gim, '<h1 class="text-3xl font-extrabold mt-10 mb-6 text-stone-100">$1</h1>');

  // Bold / Strong
  html = html.replace(/\*\*(.*?)\*\*/g, '<strong class="font-semibold text-stone-200">$1</strong>');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("<li")) {
      ...
      if (line.startsWith("<h") || line.startsWith("<blockquote") || line.startsWith("---")) {
        ...
        formattedLines.push(`<p class="leading-relaxed mb-4 text-stone-300">${line}</p>`);
      }
    }
  }
