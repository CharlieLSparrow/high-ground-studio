...
export interface LocalSessionRecordingShareStore {
  claim(input: {
    now: Date;
    leaseMs: number;
  }): Promise<LocalSessionRecordingShareClaim | null>;
  complete(input: {
    claim: LocalSessionRecordingShareClaim;
...
function authorizedTarget(
  ...
  if (!inside(configured, requested)) {
    throw Object.assign(
      new Error(
        "Session share target escaped the configured local media root.",
      ),
      { code: "session-recording-share-target-path-rejected" },
    );
  }
...
async function verifySource(
  source: SessionRecordingShareJob["sources"][number],
  resolved: string,
) {
  ...
  if (
    !sourceStat.isFile() ||
    sourceStat.size !== source.sizeBytes ||
    sha256 !== source.sha256
  ) {
    throw Object.assign(
      new Error(
        "Session share source changed after its immutable edit binding.",
      ),
      { code: "session-recording-share-source-byte-mismatch" },
    );
  }
}

function receiptMatchesJob(
  receipt: LocalSessionRecordingShareReceipt,
  job: SessionRecordingShareJob,
) {
  const metadata = receipt.customMetadata;
  const editSha256 = createHash("sha256")
    .update(JSON.stringify(job.edit))
    .digest("hex");
  const legacyFullRange =
    job.edit.keptRanges.length === 1 &&
...
async function loadReusableRender(
  ...
  try {
    const [receipt, targetStat, sha256] = await Promise.all([
      readFile(`${target}.quipsly.json`, "utf8").then(
        (value) => JSON.parse(value) as LocalSessionRecordingShareReceipt,
      ),
      stat(target),
      sha256File(target),
    ]);
    if (
      ...
      throw Object.assign(
        new Error(
          "Existing Session share does not match its immutable render receipt.",
        ),
        { code: "session-recording-share-existing-target-mismatch" },
      );
    }
...
function resultFromReceipt(
  ...
  return newSessionRecordingShareResult({
    ...
    edit: job.edit,
    sourceRecordingAssetIds: job.sources.map(
...
export async function runOneLocalSessionRecordingShareJob(
  ...
  const progress = (phase: string) =>
    process.stdout.write(
      `${JSON.stringify({ at: options.now().toISOString(), lane: JOB_TYPE, jobId: claim.id, phase })}\n`,
    );
  progress("claimed");
  ...
  try {
    ...
    await store.fail({
      claim,
      code: "session-recording-share-job-invalid",
      message:
        error instanceof Error
          ? error.message
          : "Invalid Session recording share job.",
      now: options.now(),
    });
    return {
  ...
  if (
    ...
    job.sources.some((source) => source.provider !== "local")
  ) {
    await store.fail({
      claim,
      code: "session-recording-share-provider-unsupported",
      message: "The local Session share worker accepts local media only.",
      now: options.now(),
    });
    return {
      disposition: "failed" as const,
  ...
  try {
    ...
    try {
      ...
      await copyFile(temporary, target, fsConstants.COPYFILE_EXCL).catch(
        async (error: any) => {
          ...
          if (!existing || existing.sha256 !== rendered.sha256) {
            throw Object.assign(
              new Error(
                "A different immutable Session share already occupies this target.",
              ),
              { code: "session-recording-share-existing-target-mismatch" },
            );
          }
    ...
    const receipt: LocalSessionRecordingShareReceipt = {
      ...
      customMetadata: {
        ...
        quipslyExpectedSizeBytes: String(rendered.sizeBytes),
    ...
    try {
      await writeFile(`${target}.quipsly.json`, JSON.stringify(receipt), {
        mode: 0o600,
        flag: "wx",
      });
    } catch (error: any) {
    ...
    if (!durable)
      throw new Error(
        "Session share render receipt was not durable after installation.",
      );
    ...
    const code =
      typeof error === "object" && error && "code" in error
        ? String(error.code)
        : "session-recording-share-worker-failed";
    await store.fail({
      claim,
      code,
      message:
