...
test("fails closed and removes output when an exact source changes during rendering", async () => {
  ...
  try {
    ...
    await assert.rejects(
      stat(path.resolve(fixture.root, job.reframe.target.locator)),
    );
  } finally { await rm(fixture.root, { recursive: true, force: true }); }
});

test("refuses a spatial job addressed to another executor before reading media", async () => {
  const fixture = await files();
  try {
    const job = await manifest(fixture);
    const store = new MemoryStore(job);
    let rendered = false;
    const result = await runOneLocalSpatialReframeJob(
      store,
      {
        render: async () => {
          rendered = true;
    ...
    assert.deepEqual(result, {
      disposition: "failed",
      jobId: job.jobId,
      code: "spatial-reframe-manifest-invalid",
    });
...
async function manifest(fixture: Awaited<ReturnType<typeof files>>) {
  ...
  const raw = {
    ...
    stitch: { profile: "insta360-flowstate-equirectangular-master-v1" as const, adapter: "insta360-studio-reviewed-export" as const, minimumMajorVersion: 3 as const, scope: "complete-source" as const, stitchType: "ai-flow" as const, outputProjection: "equirectangular" as const, width: 5760 as const, height: 2880 as const, videoCodec: "h265" as const, target: { provider: "local" as const, ...EXECUTION_TARGET, locator: fixture.masterPath, contentType: "video/mp4" as const }, reviewedMaster: { derivativeId: "derivative_test0001", workflowJobId: "reviewjob_test0001", receiptSha256: "4".repeat(64), adapterVersion: "5.9.9", generation: `sha256:${master.sha256}`, sha256: master.sha256, sizeBytes: master.sizeBytes, durationSeconds: 0.416667, fps: 24, videoCodec: "hevc" } },
    reframe: { adapter: "ffmpeg-v360" as const, profile: "spatial-proof-720p24" as const, commandResolution: "output-frame" as const, target: { provider: "local" as const, ...EXECUTION_TARGET, locator: path.join("output", "proof.mp4"), contentType: "video/mp4" as const } },
...
function options(fixture: Awaited<ReturnType<typeof files>>) { return { executionId: "execution_test0001", custodianNodeId: EXECUTION_TARGET.custodianNodeId, storageScopeId: EXECUTION_TARGET.storageScopeId, buildId: "test-build", imageDigest: null, leaseMs: 60_000, outputRoot: fixture.root, authorizedSourceRoots: [fixture.root], now: () => new Date("2026-08-08T02:00:00.000Z") }; }
