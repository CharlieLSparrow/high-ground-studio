...
export interface LocalExternalSourceProxyStore {
  claim(input: { executionId: string; leaseMs: number; now: Date }): Promise<LocalExternalSourceProxyClaim | null>;
  resolve(claim: LocalExternalSourceProxyClaim, job: ExternalSourceProxyJob): Promise<ResolvedExternalSource>;
  complete(input: { claim: LocalExternalSourceProxyClaim; job: ExternalSourceProxyJob; receipt: ExternalSourceProxyResult; now: Date }): Promise<boolean>;
  retry(input: { claim: LocalExternalSourceProxyClaim; code: string; message: string; now: Date }): Promise<boolean>;
  fail(input: { claim: LocalExternalSourceProxyClaim; code: string; message: string; now: Date }): Promise<boolean>;
}
...
export async function runOneLocalExternalSourceProxyJob(
  ...
  try {
    ...
    await store.fail({ claim, code: "external-proxy-job-invalid", message: message(error, "External proxy job is invalid."), now: options.now() });
    return { disposition: "failed", jobId: claim.id, code: "external-proxy-job-invalid" };
  }
  ...
  try {
    ...
    if (!sourcePath || !path.isAbsolute(sourcePath)) {
      throw new ExternalProxyTerminalError("external-proxy-source-unavailable", "The retained local source path is unavailable.");
    }
    ...
    if (existing) {
      if (!existing.isFile() || !transcoder.inspect) {
        throw new ExternalProxyTerminalError("external-proxy-existing-output-invalid", "An existing proxy cannot be independently inspected.");
      }
    ...
    if (sourceBefore.sha256 !== sourceAfter.sha256 || sourceBefore.sizeBytes !== sourceAfter.sizeBytes) {
      throw new ExternalProxyTerminalError("external-proxy-source-drift", "The original changed while its collaboration proxy was generated.");
    }
    ...
    const committed = await store.complete({ claim, job, receipt, now: options.now() });
    return committed
    ...
    if (error instanceof ExternalProxyTerminalError) {
      ...
      await store.fail({ claim, code: error.code, message: error.message, now: options.now() });
      return { disposition: "failed", jobId: job.jobId, code: error.code };
    }
    const code = error instanceof ProxyTranscodeError ? error.code : "external-proxy-worker-retry";
    const detail = message(error, "The local external-source proxy worker needs retry.");
...
export class PostgresLocalExternalSourceProxyStore implements LocalExternalSourceProxyStore {
  ...
  async resolve(_claim: LocalExternalSourceProxyClaim, job: ExternalSourceProxyJob) {
  ...
  async complete(input: { claim: LocalExternalSourceProxyClaim; job: ExternalSourceProxyJob; receipt: ExternalSourceProxyResult; now: Date }) {
    const client = await this.pool.connect();
    try {
      await client.query("BEGIN");
      const updated = await client.query({
      ...
      await client.query({
        ...
        values: [
          ...
          JSON.stringify({ schema: "quipsly-media-derivative-verification-v1", source: input.receipt.source, output: input.receipt.output, originalRemainsSourceTruth: true }),
          JSON.stringify({ schema: "quipsly-media-derivative-provenance-v1", jobId: input.job.jobId, worker: input.receipt.worker }),
      ...
      await client.query({
        ...
        values: [input.job.projectId, input.job.source.sourceRevisionId, input.job.derivativeId],
      });
      await client.query({
        ...
        values: [input.job.source.sourceRevisionId, input.receipt.output.durationSeconds, input.receipt.output.widthPixels, input.receipt.output.heightPixels, input.receipt.output.framesPerSecond],
      });
      ...
      if (
        ...
        throw new ExternalProxyTerminalError("external-proxy-derivative-conflict", "The derivative identity is already bound to different output evidence.");
      }
      await client.query("COMMIT");
      return true;
    } catch (error) {
  ...
  async retry(input: { claim: LocalExternalSourceProxyClaim; code: string; message: string; now: Date }) {
    return this.release(input, "queued");
  }

  async fail(input: { claim: LocalExternalSourceProxyClaim; code: string; message: string; now: Date }) {
    return this.release(input, "failed");
  }

  private async release(input: { claim: LocalExternalSourceProxyClaim; code: string; message: string; now: Date }, status: "queued" | "failed") {
    const current = await this.pool.query({ text: `SELECT "resultJson" FROM "StudioWorkflowJob" WHERE "id"=$1`, values: [input.claim.id] });
    ...
    const result = await this.pool.query({
      ...
      values: [
        ...
        JSON.stringify({
          ...previous,
          state: status,
          failure: { code: input.code, message: input.message, failedAt: input.now.toISOString(), attempt: input.claim.attempt },
...
function assertResolvedSource(job: ExternalSourceProxyJob, source: ResolvedExternalSource) {
  if (
    ...
    throw new ExternalProxyTerminalError("external-proxy-source-binding-changed", "The retained external source no longer matches the queued exact-revision contract.");
  }
}

async function inspectSource(sourcePath: string) {
  const details = await stat(sourcePath);
  if (!details.isFile() || details.size <= 0) throw new ExternalProxyTerminalError("external-proxy-source-unavailable", "The retained external source is empty or unavailable.");
  return { sizeBytes: details.size, sha256: await sha256File(sourcePath) };
}

function assertSourceBytes(job: ExternalSourceProxyJob, source: { sizeBytes: number; sha256: string }) {
  if (source.sizeBytes !== job.source.expectedSizeBytes || source.sha256 !== job.source.expectedContentSha256) {
    throw new ExternalProxyTerminalError("external-proxy-source-byte-mismatch", "The local file no longer matches the retained immutable source revision.");
  }
}
...
function authorizedTargetPath(root: string, locator: string) {
  ...
  if (!relative || relative.startsWith("..") || path.isAbsolute(relative) || !output.endsWith(".mp4")) {
    throw new ExternalProxyTerminalError("external-proxy-target-path-rejected", "The proxy target escaped its dedicated output root.");
  }
  return output;
}
