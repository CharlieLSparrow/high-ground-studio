...
function CharlieAsideBlock({ block }: { block: LivingManuscriptBlock }) {
  const presentation = getBlockPresentation(block);
  const anchorId = `paired-${block.id}`;

  return (
    <aside id={anchorId} className={presentation.stampClassName}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        {presentation.charlieLabel ? (
          <div className={presentation.stampChipClassName}>
            {presentation.charlieLabel}
...
function ReadingBlock({
  ...
  block: LivingManuscriptBlock;
  pairedBlocks: LivingManuscriptBlock[];
  showMetadata: boolean;
}) {
  const presentation = getBlockPresentation(block);
    ...
    <article id={block.id} className="scroll-mt-6">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[2.5fr_1fr]">
        ...
        <div className="space-y-4">
          {pairedBlocks.map((pairedBlock) => (
            <CharlieAsideBlock key={pairedBlock.id} block={pairedBlock} />
          ))}
        </div>
      </div>
    </article>
...
function EpisodeReadingView({
  ...
  const arrangedBlocks = episode.visibleBlocks;

  const pairedBlocksByTarget = arrangedBlocks
    .filter((b) => b.voice === "charlie" && b.pairsWith.length > 0)
    .reduce(
      (acc, block) => {
        block.pairsWith.forEach((targetId) => {
          if (!acc[targetId]) acc[targetId] = [];
          acc[targetId].push(block);
        });
        return acc;
      },
      {} as Record<string, LivingManuscriptBlock[]>,
    );

  const charlieBlockIdsWithPairings = new Set(
    Object.values(pairedBlocksByTarget)
      .flat()
      .map((b) => b.id),
  );
    ...
    <section id={`episode-${episode.key}`} className="scroll-mt-6">
      <PaperCard className="min-w-0 px-6 py-8 sm:px-8 sm:py-10">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex flex-wrap gap-2">
              <PageEyebrow>Episode Reading View</PageEyebrow>
              <PageEyebrow>{formatLabel(episode.status)}</PageEyebrow>
              <PageEyebrow>{episode.key}</PageEyebrow>
              {episode.primaryChapter ? (
                <PageEyebrow>{formatLabel(episode.primaryChapter)}</PageEyebrow>
      ...
      <div className="mt-6 space-y-6">
        {arrangedBlocks.length === 0 ? (
          <div className="rounded-3xl border border-[rgba(37,28,20,0.1)] bg-[rgba(255,255,255,0.38)] px-4 py-4 text-[rgba(38,30,24,0.76)]">
            No visible blocks in this episode for the current search and filters.
          </div>
        ) : (
          arrangedBlocks.map((block) => {
            ...
            if (block.voice === "homer") {
              const paired = pairedBlocksByTarget[block.id] || [];
              paired.forEach((p) => renderedBlockIds.add(p.id));
            ...
            if (
              block.voice === "charlie" &&
              !charlieBlockIdsWithPairings.has(block.id)
            ) {
              renderedBlockIds.add(block.id);
              return (
