...
describe("browser source capture contract", () => {
  test("selects the first browser-supported production container", () => {
    expect(chooseBrowserSourceMimeType("audio", (value) => value === "audio/mp4"))
      .toBe("audio/mp4");
    expect(chooseBrowserSourceMimeType("video", (value) => value.includes("vp8")))
      .toBe("video/webm;codecs=vp8,opus");
  });

  test("never presents recording as ready without durable storage and consent", () => {
    expect(browserSourceCanBegin({
      opfsAvailable: false,
      microphoneId: "mic",
      sourceType: "audio",
  ...
  test("maps GCS range receipts and 8 MiB resumable chunks without overlap", () => {
    expect(browserSourcePersistedBytes("bytes=0-8388607")).toBe(8 * 1024 * 1024);
    expect(browserSourcePersistedBytes(null)).toBe(0);
    expect(browserSourceNextUploadChunk(20 * 1024 * 1024, 8 * 1024 * 1024)).toEqual({
      start: 8 * 1024 * 1024,
      endExclusive: 16 * 1024 * 1024,
      endInclusive: 16 * 1024 * 1024 - 1,
      sizeBytes: 8 * 1024 * 1024,
    });
    expect(browserSourceNextUploadChunk(20 * 1024 * 1024, 20 * 1024 * 1024)).toBeNull();
  });
});
