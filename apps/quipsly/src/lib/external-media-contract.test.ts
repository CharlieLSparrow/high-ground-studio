...
describe("external media contract", () => {
  ...
  it("rejects invalid spatial projection and technical media metadata", () => {
    expect(() => normalizeAttachVerifiedExternalMediaInput({
      projectId: "project_01", actorUserId: "user_01", actorEmail: "a@example.test", clientRequestId: crypto.randomUUID(), operation: "attach", verifiedFile: { ...verifiedFile(), mediaProjection: "cubemap" as "flat" },
    })).toThrow("media projection is unsupported");
    expect(() => normalizeAttachVerifiedExternalMediaInput({
      projectId: "project_01", actorUserId: "user_01", actorEmail: "a@example.test", clientRequestId: crypto.randomUUID(), operation: "attach", verifiedFile: { ...verifiedFile(), widthPixels: 0 },
