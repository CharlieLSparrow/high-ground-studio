...
jest.mock("@/lib/server/audio-source-alignment-cloud", () => ({
  ensureSessionAudioSourceAlignmentCloudQueued: (...args: unknown[]) => mockEnsureCloudQueued(...args),
}));
...
function candidate(input: {
  ...
  return {
    ...
    localManifestJson: input.estimatedStart === null ? {} : {
      alignment: {
...
describe("Session exact-source audio alignment planning", () => {
  ...
  it("turns two monotonic clock proposals into two bounded waveform checks", () => {
    const plan = buildSessionSourceAlignmentPlan({
      captureGroupId,
      spine: candidate({ id: "recording_spine_1234", start: "2026-08-24T20:00:00.000Z" }),
      target: candidate({ id: "recording_target_123", start: "2026-08-24T20:00:00.350Z" }),
    });
    ...
    expect(plan.proposal.laterTargetSeconds).toBeGreaterThan(plan.proposal.openingTargetSeconds);
    expect(plan.proposal.laterTargetSeconds + plan.proposal.windowSeconds).toBeLessThanOrEqual(120);
  ...
  it("keeps wall-clock fallback visible instead of pretending it is capture evidence", () => {
    const plan = buildSessionSourceAlignmentPlan({
      captureGroupId,
      spine: candidate({ id: "recording_spine_1234", start: "2026-08-24T20:00:00.000Z", estimatedStart: null }),
      target: candidate({ id: "recording_target_123", start: "2026-08-24T20:00:01.250Z", estimatedStart: null }),
    });
  ...
  it("fails closed when retained overlap cannot support opening and drift checks", () => {
    expect(() => buildSessionSourceAlignmentPlan({
      captureGroupId,
      spine: candidate({ id: "recording_spine_1234", start: "2026-08-24T20:00:00.000Z", duration: 5 }),
      target: candidate({ id: "recording_target_123", start: "2026-08-24T20:00:04.000Z", duration: 5 }),
    })).toThrow(SessionSourceAlignmentError);
  });

  it("rejects cross-room and duplicate source identities", () => {
    const spine = candidate({ id: "recording_spine_1234", start: "2026-08-24T20:00:00.000Z" });
    expect(() => buildSessionSourceAlignmentPlan({ captureGroupId, spine, target: spine })).toThrow("two different");
    expect(() => buildSessionSourceAlignmentPlan({
      captureGroupId,
      spine,
      target: { ...candidate({ id: "recording_target_123", start: "2026-08-24T20:00:00.000Z" }), roomId: "another_room_123456" },
  ...
  it("queues only released exact-byte sources from the same canonical take", async () => {
    ...
    const assets = [asset("recording_spine_1234"), asset("recording_target_123")];
    assets[1]!.recordedStartedAt = new Date("2026-08-24T20:00:00.350Z");
    assets[1]!.localManifestJson.alignment.estimatedServerStartedAt = "2026-08-24T20:00:00.350Z";
    ...
    const prisma = {
      ...
      mobileCaptureFinalizationReceipt: { findMany: jest.fn().mockResolvedValue(assets.map((row) => receipt(row.id))) },
      sessionAudioAlignmentJob: {
        findFirst: jest.fn().mockResolvedValue(null),
        create: jest.fn(async ({ data }) => { const row = { ...data, createdAt: new Date(), updatedAt: new Date() }; created.push(row); return row; }),
        findUnique: jest.fn(async () => created[0]),
      },
    };
    mockEnsureCloudQueued.mockResolvedValue({ status: "configuration-required" });
    ...
    expect(prisma.mobileCaptureFinalizationReceipt.findMany).toHaveBeenCalledWith(expect.objectContaining({
      where: { recordingAssetId: { in: [assets[0]!.id, assets[1]!.id] }, processingDisposition: "RELEASED" },
    }));
    expect(created[0].roomId).toBe(room.id);
    expect(created[0].inputJson.kind).toBe("quipsly-session-audio-alignment-job-v1");
