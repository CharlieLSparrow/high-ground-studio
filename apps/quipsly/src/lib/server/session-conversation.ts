...
function serialize(row: any, actorUserId: string) {
  return {
    ...
    gifUrl: row.deletedAt ? null : row.gifUrl ?? null,
    ...
    author: {
      ...
      label:
        row.author?.name || row.authorNameSnapshot || row.author?.primaryEmail || row.authorEmailSnapshot || "Session participant",
    ...
    replyTo: row.replyTo
      ? {
          ...
          authorLabel:
            ...
            row.replyTo.authorNameSnapshot ||
...
export async function GET(
  ...
  const pageCursor = cursorId ? await access.prisma.sessionConversationMessage.findFirst({
    where: { id: cursorId, roomId }, select: { id: true, createdAt: true },
  }) : null;
  if (cursorId && !pageCursor) return NextResponse.json({ ok: false, error: "This conversation page is no longer available. Refresh the conversation." }, { status: 400 });
  const [latestMessages, cursor, writableRoom] = await Promise.all([
    access.prisma.sessionConversationMessage.findMany({
      where: { roomId, ...(pageCursor ? { OR: [
  ...
  const hasMore = latestMessages.length > limit;
  const messages = latestMessages.slice(0, limit).reverse();
  const nextCursor = hasMore ? messages[0]?.id ?? null : null;
  const requestedMessageId = params.get("message");
  if (requestedMessageId && !messages.some((row: any) => row.id === requestedMessageId)) {
    const anchor = await access.prisma.sessionConversationMessage.findFirst({ where: { id: requestedMessageId, roomId }, select: SELECT });
    if (anchor) {
