...
import { buildVerifiedTextSelector, createWritingDraftFromSourceAnnotation } from "./source-annotations";

describe("source annotation anchors", () => {
  it("retains quote and position evidence against immutable source text", () => {
    const immutableText = "Opening context. Keep the source intact and decisions around it. Closing context.";
  ...
  it("fails closed when a stale client selection no longer matches", () => {
    expect(buildVerifiedTextSelector({
      ...
      ok: false,
      message: "The source changed or the selection no longer matches. Reopen the source and select it again.",
    });
  });

  it("rejects empty and out-of-bounds selections", () => {
    expect(buildVerifiedTextSelector({ immutableText: "Source", startOffset: 2, endOffset: 2, exactText: "" }).ok).toBe(false);
    expect(buildVerifiedTextSelector({ immutableText: "Source", startOffset: 0, endOffset: 99, exactText: "Source" }).ok).toBe(false);
  });
});
...
describe("annotation to writing handoff", () => {
  it("creates one private draft block plus a durable evidence-use receipt", async () => {
    ...
    const tx = {
      ...
      studioDocument: { create: jest.fn().mockResolvedValue({ id: "document-db-1" }) },
      studioDocumentBlock: { create: jest.fn().mockResolvedValue({ id: "block-db-1" }) },
    ...
    expect(result).toMatchObject({ ok: true, reused: false, href: expect.stringContaining("/create?project=high-ground&document=") });
    expect(tx.studioDocument.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ projectId: "project-1", projectionStatus: "draft", isPrivate: true }) }));
    expect(tx.studioDocumentBlock.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({
      body: expect.stringContaining("> Keep the source intact."),
      externalId: "annotation:annotation-1",
      isPrivate: true,
    }) }));
    expect(tx.$executeRaw).toHaveBeenCalledTimes(1);
    expect(tx.studioDocumentOperation.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ operationType: "create-draft-from-source-annotation", reversible: true }) }));
  });
});
