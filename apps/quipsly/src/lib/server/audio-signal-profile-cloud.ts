...
import {
  ...
  type AudioSignalProfileCloudQueueReceipt,
...
export type AudioSignalProfileCloudQueueStatus = {
...
export async function ensureAudioSignalProfileCloudQueued(input: { prisma: any; processingJob: any }): Promise<AudioSignalProfileCloudQueueStatus> {
  const job = parseAudioSignalProfileJob(input.processingJob.inputJson, input.processingJob.id);
  if (job.source.provider !== "gcs") throw new Error("Audio signal profile cloud outbox requires an exact GCS source.");
  if (input.processingJob.type !== "audio-signal-profile" || input.processingJob.projectId !== job.projectId || input.processingJob.assetId !== job.source.assetId) {
    throw new Error("Audio signal profile cloud outbox no longer matches its processing job.");
  }
  const source = gcsLocation(job.source.locator, job.source.generation);
  const bucket = getMediaBucket(source.bucketName);
  const manifestObjectName = buildAudioSignalProfileCloudManifestObjectName(job.jobId);
  const queueObjectName = buildAudioSignalProfileCloudQueueObjectName(job.jobId);
  const resultObjectName = buildAudioSignalProfileCloudResultObjectName(job.jobId);
  const desired = newAudioSignalProfileCloudManifest(job);
  ...
  if (JSON.stringify(manifest.job) !== JSON.stringify(desired.job)) throw new Error("Existing audio signal profile manifest has a different immutable job binding.");
  if (manifest.status === "failed-terminal") {
    ...
    return status("failed", job.jobId, source.bucketName, manifestObjectName, queueObjectName, resultObjectName, false);
  }
  if (manifest.status !== "completed") {
    await saveQueueIfAbsent(bucket, queueObjectName, {
      kind: AUDIO_SIGNAL_PROFILE_CLOUD_QUEUE_KIND,
      version: 1,
      jobId: job.jobId,
      manifestObjectName,
  ...
  await input.prisma.studioAssetProcessingJob.update({
    where: { id: job.jobId },
    data: {
      status: manifest.status === "completed" ? "output-ready" : manifest.status === "queued" ? "queued" : "processing",
      error: null,
  ...
  if (mediaProcessorExecutionRequestIsRecent(processingControl.executionRequestedAt)) {
    return status(manifest.status === "processing" ? "processing" : "queued", job.jobId, source.bucketName, manifestObjectName, queueObjectName, resultObjectName, false);
  }
...
function status(state: AudioSignalProfileCloudQueueStatus["status"], jobId: string, bucketName: string, manifestObjectName: string, queueObjectName: string, resultObjectName: string, executionRequested: boolean): AudioSignalProfileCloudQueueStatus {
  return { status: state, jobId, bucketName, manifestObjectName, queueObjectName, resultObjectName, executionRequested };
}

async function saveManifestIfAbsent(bucket: any, objectName: string, manifest: AudioSignalProfileCloudManifest) {
  try {
    await bucket.file(objectName).save(JSON.stringify(manifest), {
      resumable: false,
      validation: "crc32c",
      contentType: "application/json; charset=utf-8",
      metadata: { cacheControl: "private, no-store", metadata: { quipslyKind: manifest.kind, quipslySignalProfileJobId: manifest.job.jobId, quipslyAssetId: manifest.job.source.assetId } },
...
async function saveQueueIfAbsent(bucket: any, objectName: string, receipt: AudioSignalProfileCloudQueueReceipt, desired: AudioSignalProfileCloudManifest) {
  try {
    await bucket.file(objectName).save(JSON.stringify(receipt), {
      resumable: false,
      validation: "crc32c",
      contentType: "application/json; charset=utf-8",
      metadata: { cacheControl: "private, no-store", metadata: { quipslyKind: receipt.kind, quipslySignalProfileJobId: receipt.jobId } },
      preconditionOpts: { ifGenerationMatch: 0 },
    });
  } catch (error) { if (!precondition(error)) throw error; }
  const canonical = parseAudioSignalProfileCloudQueueReceipt((await loadJson(bucket, objectName)).value);
  const historical = parseAudioSignalProfileCloudManifest((await loadJson(bucket, canonical.manifestObjectName, canonical.manifestGeneration)).value, canonical.jobId);
  if (JSON.stringify(historical.job) !== JSON.stringify(desired.job)) throw new Error("Existing audio signal profile queue receipt points at a different immutable job.");
}

export async function loadAudioSignalProfileCloudJsonIfPresent(bucket: any, objectName: string) {
  try { return await loadJson(bucket, objectName); }
...
function gcsLocation(locator: string, generation: string) {
  ...
  if (!match || match[3] !== generation || match[2].split("/").some((part) => !part || part === "." || part === "..")) throw new Error("Audio signal profile source must be one generation-bound media-vault object.");
  return { bucketName: match[1], objectName: match[2], generation: match[3] };
}

function requiredGeneration(value: unknown) { const generation = String(value ?? ""); if (!/^[1-9][0-9]*$/.test(generation)) throw new Error("Audio signal profile control object lacks an immutable generation."); return generation; }
