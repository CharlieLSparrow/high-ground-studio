...
describe("permission-filtered workspace search", () => {
  ...
  it("searches scoped canonical records and excludes unreviewed transcript candidates", async () => {
    ...
    const noteFindMany = jest.fn().mockResolvedValue([{ id: "note-1", title: "Proof insight", body: "Proof this next", kind: "SESSION_NOTE", updatedAt: new Date(), room: { id: "room-1", title: "Episode review" }, tagLinks: [{ tag: { id: "tag-1", slug: "proof", label: "Proof", isActive: true } }] }]);
    const prisma = {
    ...
    expect(noteFindMany).toHaveBeenCalledWith(expect.objectContaining({
      where: expect.objectContaining({
        AND: expect.arrayContaining([{ kind: { notIn: ["SUMMARY", "HIGHLIGHT"] } }]),
      }),
    }));
