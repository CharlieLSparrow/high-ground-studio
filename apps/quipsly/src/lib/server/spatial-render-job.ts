    ...
    select: {
      derivatives: {
        ...
        select: {
          ...
          provenanceJson: true,
          workflowJob: { select: { resultJson: true } },
        },
      },
    },
  },
...
export async function queueSpatialReframe(input: {
  ...
  if (existing) {
    ...
    if (
      ...
      throw new SpatialRenderQueueError(
        ...
        "That render request identity is already bound to different timeline evidence.",
      );
    }
  ...
  if (!placement || placement.status !== "active")
    throw new SpatialRenderQueueError(
      "spatial-reframe-placement-unavailable",
      "That active Episode placement no longer exists.",
      404,
    );
  const sourceSet =
    placement.sourceRange.sourceSet ??
    (await input.prisma.studioMediaSourceSet.findFirst({
      where: {
        projectId: input.projectId,
  ...
  if (!sourceSet || sourceSet.completeness !== "complete")
    throw new SpatialRenderQueueError(
      "spatial-reframe-source-set-incomplete",
      "This select is not bound to a complete spatial camera package.",
    );
  const master = sourceSet.sourceClockRevision.derivatives[0];
  if (
    !master ||
    master.widthPixels !== 5760 ||
    master.heightPixels !== 2880 ||
    !master.durationSeconds
  ) {
    throw new SpatialRenderQueueError(
      "spatial-reframe-master-required",
      "Register a reviewed 5.7K Insta360 Studio master before rendering this spatial select.",
    );
  }
  const masterReceipt = parseReviewedSpatialStitchMasterReceipt(
    record(master.workflowJob.resultJson).receipt,
  );
  ...
  if (
    ...
    throw new SpatialRenderQueueError(
      "spatial-reframe-master-evidence-mismatch",
      "The registered stitch master no longer matches its source package receipt.",
    );

  const recipe = parseRecipe(placement.sourceRange.reframeRecipeJson);
  const timeline = timelineStateFromEpisodeArtifact(
    placement.episodeProduction.timelineJson,
  );
  if (!timeline.clips.some((clip) => clip.id === placement.clipId))
    throw new SpatialRenderQueueError(
      "spatial-reframe-placement-stale",
      "The promoted card is no longer present in the canonical Episode timeline.",
    );
  const timelineFingerprintSha256 = digest(
    episodeTimelineContentFingerprint(timeline),
  );
  const members = sourceSet.members.map(({ role, sourceRevision }) => {
    try {
      ...
      if (error instanceof SpatialExactSourceError) {
        throw new SpatialRenderQueueError(
          "spatial-reframe-source-member-unavailable",
          error.message,
        );
      }
      throw error;
  ...
  const raw: Omit<SpatialRenderJob, "kind" | "version" | "boundaries"> = {
    ...
    sourcePackage: {
      ...
      sourceClockRevisionId: sourceSet.sourceClockRevisionId,
      sourceContentSha256: sourceSet.identitySha256,
      members,
    },
    selection: {
      sourceRangeId: placement.sourceRangeId,
      selectorSha256: placement.sourceRange.selectorSha256,
      startSeconds: placement.sourceRange.startSeconds,
      endSeconds: placement.sourceRange.endSeconds,
    },
    recipe,
    ...
    stitch: {
      ...
      reviewedMaster: {
        derivativeId: master.id,
        workflowJobId: master.workflowJobId,
        receiptSha256: masterReceipt.receiptSha256,
        adapterVersion: masterReceipt.review.applicationVersion,
        generation: master.generation,
        sha256: master.contentSha256,
        sizeBytes: Number(master.sizeBytes),
        durationSeconds: master.durationSeconds,
        fps: master.framesPerSecond ?? masterReceipt.output.fps,
        videoCodec: masterReceipt.output.videoCodec,
      },
    },
    reframe: {
      adapter: "ffmpeg-v360",
  ...
  const recipeSealed = {
    ...raw,
    recipeSha256: digest(spatialRecipeCanonicalJson(recipeUnsealed)),
  };
  const unsealed = newSpatialRenderJob(recipeSealed);
  const manifest = newSpatialRenderJob({
    ...recipeSealed,
    manifestSha256: digest(spatialRenderManifestCanonicalJson(unsealed)),
  });
  const created = await input.prisma.studioWorkflowJob.create({
    data: {
      ...
      priority: 50,
      requestedByEmail: input.requestedByEmail.toLowerCase(),
      inputJson: manifest as unknown as Prisma.InputJsonValue,
    },
  });
  return publicQueue(created, manifest, false);
}

export async function registerSpatialReframeResult(input: {
  prisma: PrismaClient;
  projectId: string;
  jobId: string;
  authorizedRoot: string;
}) {
  const row = await input.prisma.studioWorkflowJob.findFirst({
    where: { id: input.jobId, projectId: input.projectId },
  });
  if (!row || row.type !== JOB_TYPE || row.source !== JOB_SOURCE)
    throw new SpatialRenderQueueError(
      "spatial-reframe-job-not-found",
      "That spatial render job does not exist.",
      404,
    );
  const job = parseSpatialRenderJob(row.inputJson);
  if (row.status === "completed") {
    const derivative = await input.prisma.studioMediaDerivative.findUnique({
      where: { workflowJobId: row.id },
    });
    if (!derivative)
      throw new SpatialRenderQueueError(
        "spatial-reframe-registration-incomplete",
        "The completed spatial job is missing its derivative.",
      );
    return publicRegistration(derivative, job, true);
  }
  if (row.status !== "output-ready")
    throw new SpatialRenderQueueError(
      "spatial-reframe-output-not-ready",
      row.status === "failed"
        ? row.error || "The spatial render failed."
        : "The local spatial worker has not completed this render.",
    );
  const receipt = parseSpatialRenderResult(record(row.resultJson).receipt, job);
  const outputPath = await authorizeResultPath(
    input.authorizedRoot,
    receipt.reframe.output.locator,
  );
  const output = await inspectResult(outputPath);
  if (
    output.sha256 !== receipt.reframe.output.sha256 ||
    output.sizeBytes !== receipt.reframe.output.sizeBytes ||
    receipt.reframe.output.generation !== `sha256:${output.sha256}`
  )
    throw new SpatialRenderQueueError(
      "spatial-reframe-output-byte-mismatch",
      "The spatial render output changed before registration.",
    );

  return input.prisma.$transaction(
    async (transaction) => {
      const locked = await transaction.studioWorkflowJob.findUnique({
        where: { id: row.id },
      });
      if (!locked)
        throw new SpatialRenderQueueError(
          "spatial-reframe-job-not-found",
          "The spatial render job disappeared.",
          404,
        );
      const existing = await transaction.studioMediaDerivative.findUnique({
        where: { workflowJobId: row.id },
      });
      if (locked.status === "completed" && existing)
        return publicRegistration(existing, job, true);
      if (locked.status !== "output-ready")
        throw new SpatialRenderQueueError(
          "spatial-reframe-registration-race",
          "The spatial render changed before registration.",
        );
      const derivative = await transaction.studioMediaDerivative.create({
        data: {
          ...
          provenanceJson: {
            schema: receipt.kind,
            timelinePlacementId: job.timelinePlacementId,
            timelineFingerprintSha256: job.timelineFingerprintSha256,
            sourceRangeId: job.selection.sourceRangeId,
            selectorSha256: job.selection.selectorSha256,
...
async function authorizeResultPath(configuredRoot: string, locator: string) {
  ...
  if (
    ...
    throw new SpatialRenderQueueError(
      "spatial-reframe-output-path-rejected",
      "The spatial result escaped its authorized local media vault.",
    );
  return output;
}
async function inspectResult(filePath: string) {
  const file = await stat(filePath);
  if (!file.isFile() || file.size <= 0)
    throw new SpatialRenderQueueError(
      "spatial-reframe-output-unavailable",
      "The spatial render output is unavailable.",
    );
...
function parseRecipe(value: unknown): SpatialRenderJob["recipe"] {
  const row = record(value);
  const keyframes = Array.isArray(row.keyframes) ? row.keyframes : [];
  return {
    schema: "quipsly-360-reframe-v1",
    projection: "equirectangular",
    aspectRatio: row.aspectRatio as SpatialRenderJob["recipe"]["aspectRatio"],
    stabilization:
      row.stabilization as SpatialRenderJob["recipe"]["stabilization"],
    horizonLock: row.horizonLock as boolean,
    keyframes: keyframes as SpatialRenderJob["recipe"]["keyframes"],
  };
}
