  ...
  return input.prisma.$transaction(
    async (tx) => {
      const replay = await tx.studioMediaSourceSet.findUnique({
        ...
        include: {
          members: { orderBy: [{ role: "asc" }, { ordinal: "asc" }] },
        },
      });
      if (replay) {
        ...
        if (metadata?.requestSha256 !== requestSha256) {
          throw new SourceStoryConflictError(
            ...
            "That request identity already created a different source set.",
          );
        }
      ...
      const revisions = await tx.studioMediaSourceRevision.findMany({
        where: {
          projectId: value.projectId,
          id: { in: value.members.map((member) => member.sourceRevisionId) },
        },
        select: {
          id: true,
          identitySha256: true,
      ...
      if (revisions.length !== value.members.length) {
        throw new SourceStoryContractError(
          "source-set-project-mismatch",
          "At least one source-set member is unavailable in this Nest.",
        );
      }
      ...
      if (!clock)
        throw new SourceStoryContractError(
          ...
          "The source clock is unavailable in this Nest.",
        );
      if (!clock.durationSeconds || clock.durationSeconds <= 0) {
        throw new SourceStoryContractError(
          "source-clock-duration-missing",
          "The source clock needs verified duration before it can drive selections.",
        );
      }
      const browseMember = value.members.find(
        (member) => member.role === "browse-proxy",
      );
      if (
        value.kind === "insta360-360" &&
        (!browseMember ||
          browseMember.sourceRevisionId !== value.sourceClockRevisionId)
      ) {
        throw new SourceStoryContractError(
      ...
      if (
        value.kind === "insta360-360" &&
        !value.members.some((member) => member.role === "primary-original")
      ) {
        throw new SourceStoryContractError(
          "insta360-original-required",
          "An Insta360 set requires at least one exact original member.",
        );
      }
...
function cleanKey(value: unknown, fallback: string) {
  ...
  if (!key)
    throw new SourceStoryContractError(
      "invalid-key",
      "A stable board key is required.",
    );
  return key;
}
...
function storedRequestSha256(value: unknown) {
  ...
  const candidate =
    typeof record?.requestSha256 === "string"
      ? record.requestSha256.toLowerCase()
      : "";
...
function checksumInRecord(value: unknown): string | null {
  ...
  const generation =
    typeof record.generation === "string"
      ? record.generation.trim().toLowerCase()
      : "";
  return generation.startsWith("sha256:") && SHA256.test(generation.slice(7))
    ? generation.slice(7)
    : null;
}
...
function exactAssetChecksum(input: {
  ...
  for (const attachment of input.attachments) {
    ...
    if (!root) continue;
    const declaredPlaybackUrl =
      typeof root.playbackUrl === "string" ? root.playbackUrl : null;
    ...
    for (const candidate of [
      jsonRecord(root.output),
      jsonRecord(root.asset),
      root,
    ]) {
...
async function ensureStoryBoardSections(
  db: Database,
  input: {
    boardId: string;
    actorUserId: string;
  ...
  const existingByKey = new Map(
    existing.map((section) => [section.key, section]),
  );
  let nextSortOrder =
    existing.reduce(
      (maximum, section) => Math.max(maximum, section.sortOrder),
      -1,
    ) + 1;
  for (const groupKey of groupKeys) {
    ...
    if (section) {
      if (section.archivedAt) {
        throw new SourceStoryContractError(
          "section-archived",
          "That binder section is archived. Choose an active section or restore it deliberately before filing cards there.",
        );
      }
      continue;
    }
...
async function checkedTagIds(
  ...
  if (tags.length !== unique.length) {
    throw new SourceStoryContractError(
      "invalid-tag-scope",
      "At least one tag is unavailable in this Nest.",
    );
  }
  return tags.map((tag) => tag.id).sort();
}

async function requireAssetInProject(
  db: Database,
  projectId: string,
  mediaAssetId: string,
) {
  ...
  const attached = Boolean(
    ...
    (asset.projects.length ||
      asset.mediaBin?.projectId === projectId ||
      asset.assetAttachments.length),
  );
  if (!asset || !attached) {
    throw new SourceStoryContractError(
      "asset-project-mismatch",
      "This media source is unavailable in the selected Nest.",
    );
  }
  return asset;
}
...
async function ensureAssetRevision(input: {
  ...
  const asset = await requireAssetInProject(
    input.db,
    input.projectId,
    input.mediaAssetId,
  );
  const sizeBytes =
    asset.sizeBytes && asset.sizeBytes >= BigInt(0) ? asset.sizeBytes : null;
  ...
  const identitySha256 = sha256(
    stableSourceStoryJson({ projectId: input.projectId, registrySnapshot }),
  );
  const dimensions = parseResolution(asset.resolution);
  const sourceState =
    contentSha256 && sizeBytes && sizeBytes > BigInt(0)
      ? "checksum-bound"
      : "identity-unverified";

  const revision = await input.db.studioMediaSourceRevision.upsert({
    where: {
      projectId_identitySha256: { projectId: input.projectId, identitySha256 },
    },
    update: {},
    create: {
      projectId: input.projectId,
...
export async function createStoryBoard(input: {
  ...
  const description =
    cleanText(input.description ?? "", "Description", 10_000) || null;
  const slug = cleanKey(input.slug, title);
  const kind = cleanKey(input.kind, "story");
  const episodeProductionId = input.episodeProductionId
    ? cleanId(input.episodeProductionId, "episodeProductionId")
    : null;
    ...
    async (tx) => {
      ...
      if (episodeProductionId) {
        const episode = await tx.studioEpisodeProduction.findFirst({
          where: { id: episodeProductionId, projectId },
          select: { id: true },
        });
        if (!episode)
          throw new SourceStoryContractError(
            "episode-project-mismatch",
            "The Episode is unavailable in this Nest.",
          );
      }
...
export async function createSourceStoryCard(input: {
  ...
  return input.prisma.$transaction(
    async (tx) => {
      ...
      if (existing) {
        if (
          storedRequestSha256(existing.revisions[0]?.snapshotJson) !==
          requestSha256
        ) {
          throw new SourceStoryConflictError(
            "request-reuse-conflict",
        ...
        return {
          card: existing,
          replayed: true,
          boardRevision: placementOperation?.revision ?? null,
        };
      }
        ...
        : await (async () => {
            ...
            if (!revision?.externalReference) {
              throw new SourceStoryContractError(
                ...
                "That external source revision is unavailable in this Nest.",
              );
            }
            if (!revision.derivatives[0]) {
              throw new SourceStoryContractError(
                "external-proxy-required",
                "A verified collaboration proxy is required before marking source-clock ranges.",
              );
            }
      ...
      if (
        source.revision.durationSeconds !== null &&
        value.endSeconds > source.revision.durationSeconds + 0.001
      ) {
        throw new SourceStoryContractError(
          "range-past-source",
          "The out point is beyond the registered source duration.",
        );
      }
      const sourceSet = value.sourceSetId
      ...
      if (value.sourceSetId && !sourceSet) {
        throw new SourceStoryContractError(
          "source-set-clock-mismatch",
          "That source set does not use this exact revision as its viewing clock.",
        );
      }
      ...
      if (value.boardId) {
        ...
        if (!board)
          throw new SourceStoryContractError(
            "board-project-mismatch",
            "The board is unavailable in this Nest.",
          );
        if (board.revision !== value.expectedBoardRevision) {
          throw new SourceStoryConflictError(
            "stale-board",
            "The board changed while this card was being placed.",
            board.revision,
          );
        }
        ...
        await ensureStoryBoardSections(tx, {
          ...
          clientRequestId: value.clientRequestId,
          groupKeys: [
            ...board.placements.map((placement) => placement.groupKey),
            value.groupKey,
          ],
        });
...
export async function updateSourceStoryCard(input: {
  ...
  if (!Number.isInteger(input.expectedRevision) || input.expectedRevision < 1) {
    throw new SourceStoryContractError(
      "invalid-revision",
      "The current card revision is required.",
    );
  }
  if (!storyCardPurposes.includes(input.purpose))
    throw new SourceStoryContractError(
      "invalid-purpose",
      "The story purpose is unsupported.",
    );
  if (!storyCardStatuses.includes(input.status))
    throw new SourceStoryContractError(
      "invalid-status",
      "The card status is unsupported.",
    );
  const title = cleanText(input.title, "Title", 200, true);
  ...
  return input.prisma.$transaction(
    async (tx) => {
      ...
      if (replay) {
        ...
        if (!card)
          throw new SourceStoryContractError(
            "card-project-mismatch",
            "The story card is unavailable in this Nest.",
          );
      ...
      if (card.revision !== input.expectedRevision) {
        throw new SourceStoryConflictError(
          "stale-card",
          "This card changed on another surface.",
          card.revision,
        );
      }
      ...
      await tx.studioStoryCardTagLink.deleteMany({
...
export async function rebindSourceStoryCard(input: {
  ...
  const actorUserId = cleanId(input.actorUserId, "actorUserId");
  const requestSha256 = sha256(
    stableSourceStoryJson({ ...value, actorUserId }),
  );
    ...
    async (tx) => {
      ...
      if (replay) {
        ...
        return {
          card,
          replayed: true,
          previousSourceRangeId: value.expectedSourceRangeId,
        };
      }
      ...
      if (card.sourceRangeId !== value.expectedSourceRangeId) {
        throw new SourceStoryConflictError(
          "stale-source-range",
          "This card's source changed on another surface.",
          card.revision,
        );
      }
      ...
      if (
        ...
        throw new SourceStoryContractError(
          "range-past-source",
          "The out point is beyond the registered replacement source duration.",
        );
      }
      const selectorJson = {
        schema: "quipsly-media-time-selector-v1",
      ...
      if (range.id === card.sourceRangeId) {
        throw new SourceStoryConflictError(
          ...
          "This exact source revision and range are already current.",
          card.revision,
        );
      }
      ...
      const updated = await tx.studioStoryCard.updateMany({
        where: {
          ...
          sourceRangeId: value.expectedSourceRangeId,
        },
        data: {
          sourceRangeId: range.id,
          revision: nextRevision,
...
export async function reorderStoryBoard(input: {
  ...
  if (!Number.isInteger(input.expectedRevision) || input.expectedRevision < 1)
    throw new SourceStoryContractError(
      "invalid-revision",
      "The current board revision is required.",
    );
  if (
    !Array.isArray(input.orderedCardIds) ||
    input.orderedCardIds.length > 2_000
  )
    throw new SourceStoryContractError(
      "invalid-order",
      "The board order is malformed.",
    );
  const orderedCardIds = input.orderedCardIds.map((id) =>
    cleanId(id, "cardId"),
  );
  if (new Set(orderedCardIds).size !== orderedCardIds.length)
    throw new SourceStoryContractError(
      "duplicate-card",
      "A card appears more than once in the requested order.",
    );
  const requestSha256 = sha256(
    stableSourceStoryJson({
      projectId,
  ...
  return input.prisma.$transaction(
    async (tx) => {
      ...
      if (board.revision !== input.expectedRevision)
        throw new SourceStoryConflictError(
          "stale-board",
          "The board changed on another surface.",
          board.revision,
        );
      const currentIds = board.placements
        .map((placement) => placement.cardId)
        .sort();
      if (
        stableSourceStoryJson([...orderedCardIds].sort()) !==
        stableSourceStoryJson(currentIds)
      ) {
        throw new SourceStoryConflictError(
          "order-set-mismatch",
          "The requested order does not contain the board's exact current card set.",
          board.revision,
        );
      }
      for (const [sortOrder, cardId] of orderedCardIds.entries()) {
        await tx.studioStoryBoardPlacement.updateMany({
          where: { boardId, cardId },
          data: { sortOrder },
        });
      }
      ...
      const placementByCard = new Map(
        board.placements.map((placement) => [placement.cardId, placement]),
      );
  ...
  return input.prisma.$transaction(
    async (tx) => {
      ...
      const board = await tx.studioStoryBoard.findFirst({
        ...
        include: {
          placements: { orderBy: [{ sortOrder: "asc" }, { cardId: "asc" }] },
        },
      });
      if (!board)
        throw new SourceStoryContractError(
      ...
      const desiredCardIds = value.placements.map(
      ...
      if (desiredCardIds.length) {
        const cards = await tx.studioStoryCard.findMany({
          where: {
            projectId: value.projectId,
            archivedAt: null,
            id: { in: desiredCardIds },
          },
          select: { id: true },
        });
        if (cards.length !== desiredCardIds.length) {
          throw new SourceStoryContractError(
            "arrangement-card-scope",
            "At least one card is unavailable in this Nest.",
          );
        }
      }
      ...
      const placements = value.placements.map((placement, sortOrder) => ({
        ...placement,
        sortOrder,
      }));
      await tx.studioStoryBoardOperation.create({
    ...
    async (tx) => {
      ...
      if (replay) {
        ...
        if (storedRequestSha256(replay.snapshotJson) !== requestSha256) {
          throw new SourceStoryConflictError(
            "request-reuse-conflict",
            "That request identity already created a different section.",
            replay.revision,
          );
        }
        const snapshot = jsonRecord(replay.snapshotJson);
        const sectionId =
          typeof snapshot?.sectionId === "string" ? snapshot.sectionId : null;
        const section = sectionId
          ? await tx.studioStoryBoardSection.findUnique({
              where: { id: sectionId },
            })
          : null;
        if (!section)
          throw new SourceStoryConflictError(
            "section-receipt-missing",
            "The saved section receipt needs repair.",
            replay.revision,
          );
      ...
      if (board.sections.some((section) => section.key === value.key)) {
        throw new SourceStoryConflictError(
          ...
          "That section name already has a durable identity on this board.",
          board.revision,
        );
      }
  ...
  return input.prisma.$transaction(
    async (tx) => {
      const replay = await tx.studioStoryBoardSectionOperation.findUnique({
        ...
        include: {
          section: { include: { board: { select: { projectId: true } } } },
        },
      });
      if (replay) {
        if (
          replay.section.boardId !== value.boardId ||
          replay.section.board.projectId !== value.projectId
        ) {
          throw new SourceStoryContractError(
            "section-project-mismatch",
            "That story section is unavailable in this Nest.",
          );
        }
        if (replay.requestSha256 !== requestSha256) {
          throw new SourceStoryConflictError(
            "request-reuse-conflict",
            "That request identity already revised this section differently.",
            replay.revision,
          );
        }
      ...
      if (section.revision !== value.expectedRevision) {
        throw new SourceStoryConflictError(
          "stale-section",
          "This story section changed on another surface.",
          section.revision,
        );
      }
      ...
      const snapshot = storyBoardSectionSnapshot({
        ...section,
        title: value.title,
        synopsis: value.synopsis,
        revision: nextRevision,
    ...
    async (tx) => {
      ...
      if (replay) {
        ...
        if (storedRequestSha256(replay.snapshotJson) !== requestSha256) {
          throw new SourceStoryConflictError(
            "request-reuse-conflict",
            "That request identity already saved a different section order.",
            replay.revision,
          );
        }
        return { revision: replay.revision, replayed: true };
      ...
      if (
        stableSourceStoryJson([...value.orderedSectionIds].sort()) !==
        stableSourceStoryJson(currentIds)
      ) {
        throw new SourceStoryConflictError(
          "section-order-set-mismatch",
          "The requested order does not contain the binder's exact active section set.",
          board.revision,
        );
      }
      for (const [sortOrder, sectionId] of value.orderedSectionIds.entries()) {
        await tx.studioStoryBoardSection.updateMany({
          where: { id: sectionId, boardId: board.id, archivedAt: null },
          data: { sortOrder, updatedByUserId: actorUserId },
        });
      }
      const nextRevision = board.revision + 1;
      const updated = await tx.studioStoryBoard.updateMany({
    ...
    async (tx) => {
      ...
      if (replay) {
        ...
        if (storedRequestSha256(replay.snapshotJson) !== requestSha256) {
          throw new SourceStoryConflictError(
            "request-reuse-conflict",
            "That request identity already archived a different section.",
            replay.revision,
          );
        }
      ...
      if (section.revision !== value.expectedSectionRevision)
        throw new SourceStoryConflictError(
          "stale-section",
          "This story section changed on another surface.",
          section.revision,
      ...
      if (
        board.placements.some((placement) => placement.groupKey === section.key)
      ) {
        throw new SourceStoryContractError(
          "section-not-empty",
          "Move or unfile every card before archiving this section.",
        );
      }
      ...
      await tx.studioStoryBoardOperation.create({
        data: {
          ...
          snapshotJson: {
            ...
            requestSha256,
            sectionId: section.id,
            sectionOrder: board.sections
              .filter((candidate) => candidate.id !== section.id)
              .map((candidate) => candidate.id),
            archivedSection: storyBoardSectionSnapshot({
              ...section,
              revision: sectionRevision,
              archivedAt,
            }),
    ...
    async (tx) => {
      ...
      if (replay) {
        if (replay.requestSha256 !== requestSha256) {
          throw new SourceStoryConflictError(
            "request-reuse-conflict",
            "That request identity already opened a different writing section.",
            replay.revision,
          );
        }
        ...
        const document =
          typeof documentId === "string"
            ? await tx.studioDocument.findFirst({
                where: { id: documentId, projectId: value.projectId },
                select: { id: true, stableId: true, title: true },
              })
            : null;
        if (!document)
          throw new SourceStoryConflictError(
            "writing-document-missing",
            "The saved section writing link needs repair.",
            replay.revision,
          );
        return {
          section: { ...section, revision: replay.revision, document },
          document,
          replayed: true,
        };
      }
      ...
      if (section.document)
        return { section, document: section.document, replayed: true };
      if (section.revision !== value.expectedRevision) {
        throw new SourceStoryConflictError(
          "stale-section",
          "This story section changed on another surface.",
      ...
      const documentTitle = `${section.board.title} — ${section.title}`.slice(
        0,
        160,
      );
      ...
      if (existingDocument && existingDocument.projectId !== value.projectId) {
        throw new SourceStoryContractError(
          "writing-document-project-mismatch",
          "The section writing identity belongs to another Nest.",
        );
      }
      const documentId = existingDocument?.id ?? randomUUID();
      const document =
        existingDocument ??
        (await tx.studioDocument.create({
          data: {
            id: documentId,
            projectId: value.projectId,
            stableId,
      ...
      return {
        section: { ...section, revision: nextRevision, document },
        document,
        replayed: false,
      };
    },
...
function episodeEndSeconds(timeline: TimelineState) {
  return timeline.clips.reduce(
    (end, clip) => Math.max(end, clip.startIn + Math.max(clip.duration, 0.05)),
    0,
  );
}
...
function sourceStoryPublicPlacement<
  T extends {
    createdAt: Date;
    updatedAt: Date;
    ...
    async (tx) => {
      ...
      if (replay) {
        if (replay.operations[0]?.requestSha256 !== requestSha256) {
          throw new SourceStoryConflictError(
            "request-reuse-conflict",
            "That request identity already promoted a different Story card.",
            replay.revision,
          );
        }
        return {
          placement: sourceStoryPublicPlacement(replay),
          replayed: true,
        };
      }
      ...
      if (!episode)
        throw new SourceStoryContractError(
          "episode-project-mismatch",
          "That Episode is unavailable in this Nest.",
        );
      ...
      if (beforeFingerprint !== value.expectedTimelineFingerprint) {
        throw new SourceStoryConflictError(
          ...
          "The Episode timeline changed before this card could be promoted.",
        );
      }
      ...
      if (!card?.sourceRange)
        throw new SourceStoryContractError(
          "story-card-source-missing",
          "That Story card no longer resolves to an immutable source range.",
        );
      ...
      if (!revision.contentSha256 || !SHA256.test(revision.contentSha256)) {
        throw new SourceStoryContractError(
          ...
          "The exact Story source must be checksum-verified before timeline promotion.",
        );
      }
      if (
        !(
          revision.sourceState === "available" ||
          revision.sourceState === "checksum-bound"
        )
      ) {
        throw new SourceStoryContractError(
          "source-unavailable",
          "The exact Story source is not currently available for editing.",
        );
      }
      ...
      if (revision.externalReference && !derivative) {
        throw new SourceStoryContractError(
          "external-proxy-required",
          "A verified collaboration proxy is required before an external Story source can enter an Episode.",
        );
      }
      ...
      if (value.originBoardPlacementId) {
        ...
        if (!originBoardPlacement)
          throw new SourceStoryContractError(
            "board-placement-mismatch",
            "That card is no longer in the selected Story board position.",
          );
      } else if (value.originBoardId) {
        const board = await tx.studioStoryBoard.findFirst({
          where: {
            id: value.originBoardId,
            projectId: value.projectId,
            archivedAt: null,
          },
          select: { id: true },
        ...
        if (!board)
          throw new SourceStoryContractError(
            "board-project-mismatch",
            "That Story board is unavailable in this Nest.",
          );
      }

      const duration = Math.max(0.05, range.endSeconds - range.startSeconds);
      const episodeStartSeconds =
        value.placementMode === "append"
      ...
      const reframeRecipe = normalizeStoryReframeRecipe(
        range.reframeRecipeJson as Parameters<
          typeof normalizeStoryReframeRecipe
        >[0],
      ...
      const transforms =
        reframeRecipe?.keyframes.map((keyframe, index) => ({
      ...
      const importedMedia: EpisodeImportedMediaAsset = {
        ...
        originalName:
          revision.externalReference?.fileName ??
          revision.mediaAsset?.filename ??
          card.title,
        contentType:
          derivative?.mimeType ??
          revision.mediaAsset?.mimeType ??
          revision.externalReference?.mimeType ??
          "video/mp4",
        size: Number(
          derivative?.sizeBytes ?? revision.mediaAsset?.sizeBytes ?? BigInt(0),
        ),
        kind: "video",
        is360: Boolean(
          reframeRecipe || range.sourceSet?.kind === "insta360-360",
        ),
        originalFormat:
          revision.externalReference?.fileName
            .split(".")
            .pop()
            ?.toLowerCase() ??
          revision.mediaAsset?.filename.split(".").pop()?.toLowerCase() ??
          "",
        bucketName: "",
        ...
        metadata: {
          sourceStory: prismaJson(sourceStory) as Record<string, unknown>,
        },
        sync: {
          status: "synced",
          anchorTimelineSeconds: episodeStartSeconds,
          targetClipId: clipId,
          source: "source-story-promotion",
          syncedAt: promotedAt,
        },
        proxy: derivative
          ? {
      ...
      const nextImported = [
        ...priorImported.filter((asset) => asset.id !== importedAssetId),
        importedMedia,
      ];
    ...
    async (tx) => {
      ...
      if (replay) {
        if (replay.requestSha256 !== requestSha256)
          throw new SourceStoryConflictError(
            "request-reuse-conflict",
            "That request identity already changed this placement differently.",
            placement.revision,
          );
        return {
          placement: sourceStoryPublicPlacement(placement),
      ...
      if (placement.revision !== value.expectedRevision)
        throw new SourceStoryConflictError(
          ...
          "That timeline placement changed before it could be withdrawn.",
          placement.revision,
        );
      if (placement.status !== "active")
        throw new SourceStoryConflictError(
          "timeline-placement-not-active",
          "That Story clip is already withdrawn.",
          placement.revision,
        );
      ...
      if (beforeFingerprint !== value.expectedTimelineFingerprint)
        throw new SourceStoryConflictError(
          "stale-episode-timeline",
          "The Episode timeline changed before this Story clip could be withdrawn.",
        );
      if (!timeline.clips.some((clip) => clip.id === placement.clipId))
        throw new SourceStoryConflictError(
      ...
      const nextTimeline = {
        ...timeline,
        clips: timeline.clips.filter((clip) => clip.id !== placement.clipId),
      };
      const storedClip = jsonRecord(placement.timelineClipJson);
      const importedAssetId =
        typeof storedClip?.assetId === "string" ? storedClip.assetId : "";
      const assetStillUsed = nextTimeline.clips.some(
        (clip) => clip.assetId === importedAssetId,
      );
      const nextImported = (currentArtifact?.importedMedia ?? []).filter(
        (asset) => assetStillUsed || asset.id !== importedAssetId,
      );
...
export async function readSourceStoryWorkspace(
  ...
  const [
    ...
    prisma.studioWorkflowJob.findMany({
      ...
      select: {
        ...
        createdAt: true,
        updatedAt: true,
        mediaDerivative: { select: derivativeSelect },
      },
    }),
    prisma.studioExternalMediaReference.count({ where: { projectId } }),
    prisma.studioMediaSourceSet.count({ where: { projectId } }),
  ]);
  const sourceAudioNavigationJobs = await prisma.studioWorkflowJob.findMany({
    where: {
      projectId,
  ...
  const publicDerivative = (
    ...
    derivative
      ...
      : null;
  const jobBySourceRevisionId = new Map<
    string,
    {
      id: string;
      status: string;
  ...
  for (const job of externalProxyJobs) {
    ...
    const sourceRevisionId =
      typeof source?.sourceRevisionId === "string"
        ...
        : "";
    if (!sourceRevisionId || jobBySourceRevisionId.has(sourceRevisionId))
      continue;
  ...
  for (const job of sourceVisualJobs) {
    ...
    const sourceRevisionId =
      typeof source?.sourceRevisionId === "string"
        ? source.sourceRevisionId
        : "";
    if (!sourceRevisionId || visualJobBySourceRevisionId.has(sourceRevisionId))
      continue;
    const result = jsonRecord(job.resultJson);
    const failure = jsonRecord(result?.failure);
  ...
  >();
  for (const job of sourceAudioNavigationJobs) {
    const manifest = jsonRecord(job.inputJson);
    const source = jsonRecord(manifest?.source);
    const sourceRevisionId =
      typeof source?.sourceRevisionId === "string"
          ...
          sourceRevision: {
            ...
            externalReference: card.sourceRange.sourceRevision.externalReference
              ? {
                  ...
                  lastVerifiedAt:
                    card.sourceRange.sourceRevision.externalReference.lastVerifiedAt?.toISOString() ??
                    null,
                }
              : null,
  ...
  return {
    ...
    sourceInventoryWindow: {
      externalSources: {
        loaded: externalSources.length,
        total: externalSourceTotal,
      },
      sourceSets: { loaded: sourceSets.length, total: sourceSetTotal },
      windowLimit: 80,
      complete:
        externalSources.length === externalSourceTotal &&
        sourceSets.length === sourceSetTotal,
    },
    ...
    spatialRenderJobs: spatialRenderJobs.map((job) => {
      ...
      return {
        ...
        status: job.status,
        timelinePlacementId:
          typeof manifest?.timelinePlacementId === "string"
            ? manifest.timelinePlacementId
            : "",
        timelineFingerprintSha256:
          typeof manifest?.timelineFingerprintSha256 === "string"
    ...
    boards: boards.map((board) => ({
      ...
      placements: board.placements.map((placement) => ({
        ...
        sortOrder: placement.sortOrder,
        card: projectCard(
          (cardById.get(placement.cardId) ?? placement.card) as CardRow,
        ),
      })),
    })),
  };
