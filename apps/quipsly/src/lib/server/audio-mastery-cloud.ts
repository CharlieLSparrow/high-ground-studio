...
import {
  ...
  type AudioMasteryCloudQueueReceipt,
...
export type AudioMasteryCloudQueueStatus = {
...
export async function ensureAudioMasteryCloudQueued(input: { prisma: any; processingJob: any }): Promise<AudioMasteryCloudQueueStatus> {
  const job = parseAudioMasteryJob(input.processingJob.inputJson, input.processingJob.id);
  if (job.source.provider !== "gcs" || job.target.provider !== "gcs") throw new Error("Audio mastery cloud outbox requires an exact GCS source.");
  if (input.processingJob.type !== "audio-mastery" || input.processingJob.projectId !== job.projectId || input.processingJob.assetId !== job.source.assetId) {
    throw new Error("Audio mastery cloud outbox no longer matches its processing job.");
  }
  const source = gcsLocation(job.source.locator, job.source.generation);
  const bucket = getMediaBucket(source.bucketName);
  const manifestObjectName = buildAudioMasteryCloudManifestObjectName(job.jobId);
  const queueObjectName = buildAudioMasteryCloudQueueObjectName(job.jobId);
  const resultObjectName = buildAudioMasteryCloudResultObjectName(job.jobId);
  const desired = newAudioMasteryCloudManifest(job);
  ...
  if (JSON.stringify(manifest.job) !== JSON.stringify(desired.job)) throw new Error("Existing audio mastery manifest has a different immutable job binding.");
...
function status(
  state: AudioMasteryCloudQueueStatus["status"],
  jobId: string,
  bucketName: string,
  manifestObjectName: string,
  queueObjectName: string,
  resultObjectName: string,
  executionRequested: boolean,
): AudioMasteryCloudQueueStatus {
  return { status: state, jobId, bucketName, manifestObjectName, queueObjectName, resultObjectName, executionRequested };
}

async function saveManifestIfAbsent(bucket: any, objectName: string, manifest: AudioMasteryCloudManifest) {
  try {
    await bucket.file(objectName).save(JSON.stringify(manifest), {
      resumable: false,
      validation: "crc32c",
      contentType: "application/json; charset=utf-8",
      metadata: { cacheControl: "private, no-store", metadata: { quipslyKind: manifest.kind, quipslyMasteryJobId: manifest.job.jobId, quipslyAssetId: manifest.job.source.assetId } },
...
async function saveQueueIfAbsent(bucket: any, objectName: string, receipt: AudioMasteryCloudQueueReceipt, desired: AudioMasteryCloudManifest) {
  try {
    await bucket.file(objectName).save(JSON.stringify(receipt), {
      resumable: false,
      validation: "crc32c",
      contentType: "application/json; charset=utf-8",
      metadata: { cacheControl: "private, no-store", metadata: { quipslyKind: receipt.kind, quipslyMasteryJobId: receipt.jobId } },
      preconditionOpts: { ifGenerationMatch: 0 },
    });
  } catch (error) { if (!precondition(error)) throw error; }
  const canonical = parseAudioMasteryCloudQueueReceipt((await loadJson(bucket, objectName)).value);
  const historical = parseAudioMasteryCloudManifest((await loadJson(bucket, canonical.manifestObjectName, canonical.manifestGeneration)).value, canonical.jobId);
  if (JSON.stringify(historical.job) !== JSON.stringify(desired.job)) throw new Error("Existing audio mastery queue receipt points at a different immutable job.");
}
...
function gcsLocation(locator: string, generation: string) {
  ...
  if (!match || match[3] !== generation || match[2].split("/").some((part) => !part || part === "." || part === "..")) throw new Error("Audio mastery cloud source must be one generation-bound media-vault object.");
  return { bucketName: match[1], objectName: match[2], generation: match[3] };
}

function requiredGeneration(value: unknown) { const generation = String(value ?? ""); if (!/^[1-9][0-9]*$/.test(generation)) throw new Error("Audio mastery control object lacks an immutable generation."); return generation; }
