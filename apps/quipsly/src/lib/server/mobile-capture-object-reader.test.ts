...
describe("immutable mobile Capture object reader", () => {
  ...
  it("checks remote metadata before downloading a GCS object", async () => {
    ...
    const getMetadata = jest
      .fn()
      .mockResolvedValue([{ size: "2048", metadata: {} }]);
  ...
  it("pins a remote recording to its immutable GCS generation", async () => {
    const bytes = Buffer.from("generation-bound share");
    const sha256 = createHash("sha256").update(bytes).digest("hex");
    const file = jest.fn(() => ({
      getMetadata: jest
        .fn()
        .mockResolvedValue([
          { generation: "202", size: String(bytes.length), metadata: {} },
        ]),
      download: jest.fn().mockResolvedValue([bytes]),
    }));
    jest.mocked(getMediaBucket).mockReturnValue({ file } as any);
    await expect(
      readMobileCaptureObjectBytes({
        bucketName: "capture-production",
        objectName: "media-vault/derived/share.m4a",
        expectedByteSize: bytes.length,
        expectedSha256: sha256,
        expectedGeneration: "202",
        maxBytes: 1024,
      }),
    ).resolves.toEqual(bytes);
    expect(file).toHaveBeenCalledWith("media-vault/derived/share.m4a", {
      generation: "202",
    });
  });
});
