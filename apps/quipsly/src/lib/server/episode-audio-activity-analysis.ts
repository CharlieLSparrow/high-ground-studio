...
export async function loadEpisodeAudioActivityAnalysisContext(input: { prisma: any; projectSlug: string; episodeProductionId: string }) {
  ...
  const [decisionRows, participants, assets, sources, recordings] = await Promise.all([
    ...
    assetIds.length ? input.prisma.studioMediaAsset.findMany({ where: { id: { in: assetIds }, isProxy: false }, select: { id: true, filename: true, url: true, mimeType: true, duration: true, isProxy: true, assetAttachments: { where: { projectId: project.id }, select: { metadataJson: true } }, processingJobs: { where: { type: { in: ["audio-signal-profile", "source-transcript", "source-transcript-v2", "audio-alignment"] } }, orderBy: [{ createdAt: "desc" }, { id: "desc" }], take: 25 }, transcriptJobs: { where: { episodeProductionId: episode.id }, orderBy: [{ createdAt: "desc" }, { id: "desc" }], take: 10, select: { id: true, status: true, _count: { select: { segments: true, words: true } } } } } }) : [],
