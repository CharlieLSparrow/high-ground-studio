...
export async function createNestConversationTask(input: {
  ...
  messageId: string; title: string; clientRequestId: string; tagIds: string[];
}) {
  ...
  if (!actorUserId || !projectSlug || projectSlug.length > 200 || !/^[a-zA-Z0-9_-]{1,240}$/.test(messageId)
    ...
    || tagIds.length !== input.tagIds.length || tagIds.some(id => !/^[a-zA-Z0-9_-]{1,200}$/.test(id))) {
  ...
  const fingerprint = createHash("sha256").update(JSON.stringify([projectSlug, messageId, title, tagIds])).digest("hex");
