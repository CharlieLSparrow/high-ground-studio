...
export function CallNotesPanel({ roomId, active, onOpenWorkspace, onAttentionChange }: {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (actorRecoveryKey && recoveryKey.current !== actorRecoveryKey) {
        ...
        if (recoveryKey.current) { setDrafts({}); draftsRef.current = {}; attempts.current.clear(); setSelected(null); }
      ...
      setDrafts(current => Object.fromEntries(Object.entries(current).map(([key, draft]) => {
        ...
        return [key, latest && draft.version === draft.savedVersion && !attempts.current.has(key) && savingRef.current !== key
          ? { ...draftFor(latest), key } : draft];
      })));
    } catch (error) {
      if (alive.current) setLoadError(error instanceof Error ? error.message : "Notes couldn’t load.");
    }
  }, [roomId]);
  ...
  const update = (key: string, change: Partial<Draft>) => setDrafts(current => ({ ...current, [key]: { ...current[key], ...change, version: current[key].version + 1 } }));

  const save = useCallback(async (key: string, retry = false) => {
    const draft = draftsRef.current[key];
    if (!draft || savingRef.current || draft.conflict || (!retry && draft.error) || !draft.body.trim() || draft.version === draft.savedVersion) return;
    ...
    try {
      ...
      const payload = await response.json();
      if (!alive.current || recoveryKey.current !== attemptActor) return;
      if (!response.ok || !payload.ok || !payload.note) {
        if (payload.code === "CONFLICT" && payload.current) {
          attempts.current.delete(key);
          setDrafts(current => ({ ...current, [key]: { ...current[key], error: null, conflict: payload.current } }));
          return;
        }
      ...
      if (alive.current && recoveryKey.current === attemptActor) setDrafts(current => ({ ...current, [key]: { ...current[key], error: error instanceof Error ? error.message : "Not saved yet. Your draft is still here." } }));
    } finally {
  ...
  useEffect(() => {
    ...
    const next = Object.values(drafts).find(draft => draft.version !== draft.savedVersion && draft.body.trim() && !draft.error && !draft.conflict);
  ...
  return <div className="flex min-h-0 flex-col gap-4">
    ...
    {draft ? <>
      ...
      {draft.error ? <div role="alert" className="rounded-xl border border-destructive/30 p-3 text-sm"><p>{draft.error}</p><button type="button" onClick={() => void save(draft.key, true)} className="mt-2 min-h-11 rounded-xl border border-border px-3 font-semibold">Retry save</button></div> : null}
      {draft.conflict ? <section className="rounded-xl border border-border p-3 text-sm" aria-label="Note updated elsewhere">
