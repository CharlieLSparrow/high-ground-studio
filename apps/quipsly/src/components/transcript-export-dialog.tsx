...
export function TranscriptExportDialog({title, segments, disabled = false}: {
  ...
  const timed = transcriptHasSubtitleTiming(segments);
  const subtitle = format === "srt" || format === "vtt";
  const output = useMemo(() => {
    if (!open || disabled || !segments.length || (subtitle && !timed)) return null;
    return createTranscriptExport({title, segments, format, timestamps, speakers});
  }, [open, disabled, segments, subtitle, timed, title, format, timestamps, speakers]);
  ...
  return <>
    <button type="button" disabled={disabled || !segments.length} onClick={() => setOpen(true)} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50"><Download size={16} />Export transcript</button>
    <dialog ref={dialog} aria-labelledby={headingId} onCancel={() => setOpen(false)} onClose={() => setOpen(false)} className="m-auto max-h-[90dvh] w-[min(36rem,calc(100%-2rem))] overflow-y-auto rounded-2xl border border-border bg-background p-5 text-foreground shadow-xl backdrop:bg-black/50">
      {open && <>
        ...
        <p className="mt-2 text-sm text-muted-foreground">Includes your text corrections. Times refer to the recording timeline shown here, not a trimmed export. Export includes the full available transcript, even when you’re searching.</p>
        ...
        {!timed && <p className="text-sm text-muted-foreground">Some passages don’t have usable timing. Text formats are available; subtitles need timed passages.</p>}
