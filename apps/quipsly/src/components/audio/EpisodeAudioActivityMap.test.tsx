...
function map(clock = true): ActivityMap {
  const cells = Array.from({ length: 180 }, (_, index) => ({ index, programStartSeconds: index / 30, programEndSeconds: (index + 1) / 30, sourceSeconds: index / 30, rmsDbfs: index >= 60 && index < 90 ? -20 : -80, intensity: index >= 60 && index < 90 ? 0.75 : 0, energyActive: index >= 60 && index < 90, clippingObserved: false, providerWordActive: index >= 65 && index < 85 }));
  return {
    ...
    lanes: [{ assetId: "asset-a", sourceId: "source-a", title: "Homer iPhone.wav", kind: "dialogue", role: "dialogue-primary", participantId: "homer", participantLabel: "Homer Sparrow", mixDisposition: "include", alignment: clock ? "program-clock" : "unavailable", programOffsetSeconds: clock ? 0 : null, sourceDurationSeconds: 6, activityThresholdDbfs: -32, evidenceJobId: "signal-a", transcriptEvidenceJobId: "transcript-a", transcriptWordCount: 12, cells, agreement: { comparableCellCount: 180, agreementCellCount: 170, bothActiveCellCount: 20, energyOnlyCellCount: 10, transcriptOnlyCellCount: 0, agreementRatio: 170 / 180 } }],
    ...
    coverage: { trackCount: 1, profiledTrackCount: 1, plottedTrackCount: clock ? 1 : 0, missingProfileCount: 0, unalignedProfileCount: clock ? 0 : 1, unidentifiedDialogueTrackCount: 0, transcribedTrackCount: 1, comparableTranscriptEnergyTrackCount: clock ? 1 : 0 },
    transcriptEnergyAgreement: { comparableCellCount: clock ? 180 : 0, agreementCellCount: clock ? 170 : 0, bothActiveCellCount: clock ? 20 : 0, energyOnlyCellCount: clock ? 10 : 0, transcriptOnlyCellCount: 0, agreementRatio: clock ? 170 / 180 : null },
...
describe("EpisodeAudioActivityMap", () => {
  it("keeps measured-energy language explicit and opens a listen-first region", () => {
    ...
    expect(screen.getByText(/Energy is not speech; timed words are not VAD or measured transcript accuracy/i)).toBeInTheDocument();
