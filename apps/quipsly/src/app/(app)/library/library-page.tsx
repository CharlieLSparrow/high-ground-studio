  ...
  try {
    ...
    return <main className="mx-auto max-w-[1420px] space-y-7 px-2 py-2 text-[#3d3122]">
      ...
      <section aria-label="Library filters" className="rounded-3xl border border-[#e5d5b7] bg-white p-4 shadow-sm md:p-5">
        <form method="GET" action="/library" className="flex flex-col gap-3 md:flex-row md:items-center"><label className="relative flex-1"><span className="sr-only">Search Library</span><Search className="absolute left-3 top-3 h-5 w-5 text-[#927b5b]" aria-hidden="true" /><input name="q" defaultValue={query} maxLength={200} placeholder="Search your writing, transcripts, sources, and media…" className="w-full rounded-xl border border-[#d9c7a5] bg-[#fffdf8] py-2.5 pl-10 pr-3 text-sm font-semibold" /></label>{kind !== "ALL" && <input type="hidden" name="kind" value={kind} />}<button type="submit" className="rounded-xl bg-[#3e2f21] px-5 py-3 text-xs font-black uppercase tracking-wide text-white">Search</button></form>
        <nav aria-label="Library kinds" className="mt-4 flex flex-wrap gap-2">{(["ALL", "NOTE", "DOCUMENT", "SESSION", "SOURCE", "MEDIA", "SAVED"] as const).map((value) => <Link key={value} href={filterHref(value, query)} aria-current={kind === value ? "page" : undefined} className={`rounded-full border px-3 py-2 text-[10px] font-black uppercase tracking-wide ${kind === value ? "border-[#3e2f21] bg-[#3e2f21] text-white" : "border-[#d9c7a5] bg-[#fffaf3] text-[#765f40]"}`}>{({ ALL: "All", NOTE: "Notes", DOCUMENT: "Writing", SESSION: "Sessions", SOURCE: "Research", MEDIA: "Media", SAVED: "Saved" } as const)[value]}</Link>)}</nav>
      </section>
