...
import { createProject, createStoryboard, createStoryboardFrame, updateStoryboard, generateFrameImage } from '../actions';
    ...
    <div className="flex w-full h-full divide-x divide-zinc-200 dark:divide-zinc-800 relative">
      ...
      {activeProject?.storyboards?.[0] && (
        <StoryboardAssistantSuggestor 
          ...
          onApprove={(frames) => {
            showToast("Ledger Approved: Injecting " + frames.length + " frames via StudioAssistantLedger.", "success");
          }}
        />
      )}
      ...
      <div className="w-80 flex flex-col bg-zinc-50 dark:bg-zinc-950 overflow-y-auto">
        <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center bg-white dark:bg-zinc-900">
          ...
          <button 
            ...
            className="flex items-center gap-1 text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white px-2 py-1.5 rounded-lg transition-colors focus:ring-2 focus:ring-indigo-500 outline-none"
          >
