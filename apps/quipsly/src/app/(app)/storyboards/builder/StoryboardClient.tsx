...
import { Film, Clapperboard, Plus, Image as ImageIcon, AlignLeft, Camera, Settings2, Loader2 } from 'lucide-react';
import { createProject, createScene, createShot, updateShot, generateShotImage } from '../actions';
...
export function StoryboardClient({ initialProjects }: Props) {
  ...
  const handleAddScene = async () => {
    ...
    const sceneNumber = `${(activeProject.scenes?.length || 0) + 1}`;
  ...
  const handleAddShot = async (sceneId: string, currentShotCount: number) => {
    const shotNumber = `1.${currentShotCount + 1}`;
  ...
  const handleGenerateShot = async (shotId: string, sceneId: string) => {
    setGeneratingShots(prev => ({ ...prev, [shotId]: true }));
    const res = await generateShotImage(shotId);
    ...
    if (res.success && res.shot) {
      if (activeProject) {
        const updatedScenes = activeProject.scenes.map((s: any) => {
          if (s.id === sceneId) {
            return {
              ...s,
              shots: s.shots.map((shot: any) => shot.id === shotId ? res.shot : shot)
            };
          }
          return s;
        });
      ...
      alert("Failed to generate image: " + res.error);
    }
    setGeneratingShots(prev => ({ ...prev, [shotId]: false }));
  };
        ...
        {!activeProject ? (
          ...
          <>
            <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 flex justify-between items-center">
              <div>
                <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">{activeProject.name}</h1>
                          ...
                          <div className="sm:w-1/2 aspect-video bg-zinc-100 dark:bg-zinc-900 flex flex-col items-center justify-center border-b sm:border-b-0 sm:border-r border-zinc-200 dark:border-zinc-800 relative">
                            {shot.imageUrl ? (
                              ...
                              <>
                                ...
                                <button 
                                  onClick={() => handleGenerateShot(shot.id, scene.id)}
                                  disabled={generatingShots[shot.id]}
                                  className="mt-3 text-xs bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 font-semibold px-3 py-1.5 rounded-full hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors disabled:opacity-50 flex items-center gap-1">
                                  {generatingShots[shot.id] ? <><Loader2 className="w-3 h-3 animate-spin" /> Generating...</> : "Generate Shot"}
                                </button>
                              </>
                            )}
                          </div>
