...
function AudioSignalEvidencePanel({
  ...
  return <div className="mt-4 rounded-xl border border-sky-200 bg-white p-4" aria-label="Decoded audio signal evidence">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><p className="text-xs font-black uppercase tracking-wide text-sky-950">Decoded signal scan</p><p className="mt-1 text-xs font-semibold leading-5 text-[#765f40]">{signal.loudness?.integratedLoudnessLufs !== null && signal.loudness?.integratedLoudnessLufs !== undefined ? "Programme loudness is measured across the complete decoded source using ITU-R BS.1770-5. Sample peak is not true peak, and possible dropout always requires listening." : "Complete-frame RMS and sample-peak observations. RMS dBFS is not perceptual LUFS, and possible dropout always requires listening."}</p></div>
    ...
    <dl className="mt-4 grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-5">
      <div className="rounded-lg bg-sky-50 p-3"><dt className="font-black uppercase tracking-wide text-sky-800">{signal.loudness?.integratedLoudnessLufs !== null && signal.loudness?.integratedLoudnessLufs !== undefined ? "Programme loudness" : "RMS"}</dt><dd className="mt-1 text-lg font-black text-sky-950">{signal.loudness?.integratedLoudnessLufs !== null && signal.loudness?.integratedLoudnessLufs !== undefined ? `${signal.loudness.integratedLoudnessLufs.toFixed(1)} LUFS` : `${signal.rmsDbfs.toFixed(1)} dBFS`}</dd><dd className="mt-1 text-[10px] font-bold text-sky-800">{signal.loudness?.integratedLoudnessLufs !== null && signal.loudness?.integratedLoudnessLufs !== undefined ? "complete-source BS.1770-5" : "not LUFS"}</dd></div>
      {signal.loudness?.maximumMomentaryLoudnessLufs !== null && signal.loudness?.maximumMomentaryLoudnessLufs !== undefined ? <div className="rounded-lg bg-indigo-50 p-3"><dt className="font-black uppercase tracking-wide text-indigo-800">Loudest moment</dt><dd className="mt-1 text-lg font-black text-indigo-950">{signal.loudness.maximumMomentaryLoudnessLufs.toFixed(1)} LUFS</dd><dd className="mt-1 text-[10px] font-bold text-indigo-800">maximum 400 ms block</dd></div> : null}
