    ...
    <section id="recording-share" className="rounded-3xl border border-sky-200 bg-sky-50/40 p-5 shadow-sm sm:p-6" aria-labelledby="recording-share-heading">
      ...
      {coach && (!output || editing) ? (
        <div className="mt-5 space-y-5">
          ...
          {editableTranscript.length ? <fieldset className="rounded-2xl border border-sky-200 bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><legend className="flex items-center gap-2 text-sm font-black text-sky-950"><FileText size={16} />Edit by transcript</legend><p className="mt-1 text-xs font-semibold text-sky-800">Uncheck a passage to remove it. Quipsly keeps passages where another person is speaking.</p></div>{excludedTranscriptSegments.length ? <button type="button" onClick={() => setExcludedTranscriptKeys(new Set())} className="rounded-lg border border-sky-200 bg-sky-50 px-2.5 py-1.5 text-[11px] font-black text-sky-900"><RotateCcw className="mr-1 inline" size={12} />Include all</button> : null}</div><div className="mt-3 max-h-80 space-y-2 overflow-y-auto pr-1">{editableTranscript.map((segment) => { const key = `${segment.transcriptJobId}:${segment.segmentId}`; const included = !excludedTranscriptKeys.has(key); const safe = segment.cutSafety === "safe" && Boolean(segment.timingFingerprint); return <label key={key} className={`grid grid-cols-[auto_4.5rem_1fr] gap-3 rounded-xl border p-3 ${!safe ? "cursor-not-allowed border-amber-200 bg-amber-50" : included ? "cursor-pointer border-sky-100 bg-sky-50/50" : "cursor-pointer border-rose-200 bg-rose-50 text-rose-950"}`}><input type="checkbox" className="mt-1 size-4 accent-sky-700" checked={included} disabled={!safe} aria-describedby={!safe ? `${key}-safety` : undefined} onChange={() => setExcludedTranscriptKeys((current) => { const next = new Set(current); if (next.has(key)) next.delete(key); else next.add(key); return next; })} /><span className="pt-0.5 text-[11px] font-black tabular-nums text-sky-700">{time(segment.startSeconds)}</span><span><span className="flex flex-wrap items-center gap-2 text-xs font-black text-sky-950">{segment.speakerLabel}{safe ? <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[9px] uppercase text-emerald-900">Word timed</span> : <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[9px] uppercase text-amber-950">Kept safe</span>}</span><span className={`mt-0.5 block text-sm leading-5 ${included ? "text-sky-950" : "line-through decoration-rose-500"}`}>{segment.text}</span>{!safe ? <span id={`${key}-safety`} className="mt-1 block text-[11px] font-bold leading-4 text-amber-900">{segment.cutSafetyReason || "Precise source timing is unavailable, so this passage stays included."}</span> : null}</span></label>; })}</div><p className="mt-3 text-xs font-bold text-sky-800">{excludedTranscriptSegments.length ? `${excludedTranscriptSegments.length} passage${excludedTranscriptSegments.length === 1 ? "" : "s"} removed · ${time(durationEstimate.removedSeconds)} cut · preview about ${time(durationEstimate.previewSeconds)}.` : "Everything in the selected range is included."}</p></fieldset> : <div className="rounded-xl border border-sky-200 bg-white p-4"><p className="text-sm font-black text-sky-950">Edit by transcript appears when the transcript is ready</p><p className="mt-1 text-xs font-semibold text-sky-800">You can trim the beginning and end now.</p></div>}
