...
import { Download, FileAudio, LockKeyhole, RefreshCw, Scissors, Send, ShieldCheck, Undo2 } from "lucide-react";
...
type Output = {
  ...
  body: { edit?: { startSeconds?: number; endSeconds?: number } };
};
...
type Snapshot = {
  ...
  available?: { programDurationSeconds: number; sources: Source[] };
  output?: Output | null;
...
export function SessionRecordingShareCard({ roomId }: { roomId: string }) {
  ...
  async function mutate(action: "PREPARE" | "RELEASE" | "REVOKE") {
    ...
    try {
      ...
      if (action === "PREPARE") Object.assign(body, { title, sourceIds: [...selected], startSeconds, endSeconds });
      else {
