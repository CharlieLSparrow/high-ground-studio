...
import { BookOpen, CalendarDays, FileText, Highlighter, ListChecks, Search, Tags, Target } from "lucide-react";
  ...
  try {
    ...
    return <main className="mx-auto max-w-7xl space-y-6 px-1 py-4 text-[#3d3122]">
      ...
      {query.length >= 2 ? <div className="grid gap-4 lg:grid-cols-2">
        ...
        <ResultSection title="Tags" icon={Tags}>{result.tags.length ? <ul className="space-y-2">{result.tags.map((item) => <li key={item.id}><Link href={researchHref(item.label)} className="block rounded-xl border border-fuchsia-100 bg-fuchsia-50/40 p-3 hover:border-fuchsia-300"><strong>{item.label}</strong><span className="mt-1 block text-xs font-bold text-[#806a4d]">{item.project.name} · {String(item.category).replaceAll("_", " ")}{item.isPrivate ? " · private taxonomy" : ""}</span>{item.description ? <span className="mt-1 line-clamp-2 block text-xs font-semibold text-[#765f40]">{item.description}</span> : null}</Link></li>)}</ul> : <Empty />}</ResultSection>
      </div> : null}
