...
export default async function FindPage({ searchParams }: FindPageProps) {
  ...
  try {
    ...
    return <main className="mx-auto max-w-7xl space-y-6 px-1 py-4 text-[#3d3122]">
      <header className="rounded-[2rem] border border-[#dfcba6] bg-[radial-gradient(circle_at_top_right,_#dff5ff,_transparent_42%),linear-gradient(135deg,#fffaf0,#f8edda)] p-6 shadow-sm md:p-8"><p className="text-xs font-black uppercase tracking-[0.22em] text-[#8a653d]">{focusedTag ? "Canonical tag focus" : "Permission-filtered workspace search"}</p><h1 className="mt-2 font-serif text-4xl font-black md:text-5xl">{focusedTag ? `#${focusedTag.resolvedLabel}` : "Search all of Quipsly"}</h1><p className="mt-3 max-w-3xl text-sm font-semibold leading-6 text-[#715a3e]">{focusedTag ? `Everything below is linked to this exact tag identity in ${focusedTag.project.name}. Same-label tags in other Nests are not mixed in.` : "Find the same task, goal, Session, note, source, annotation, document, or project tag—then open its canonical record. Results never broaden your existing access."}</p>{focusedTag?.redirected ? <p className="mt-3 rounded-xl border border-fuchsia-200 bg-fuchsia-50 px-4 py-3 text-sm font-bold text-fuchsia-950" role="note">#{focusedTag.requestedLabel} was merged into #{focusedTag.resolvedLabel}. This view follows the preserved redirect to the canonical tag.</p> : null}<form action="/find" method="get" className="mt-6 flex flex-col gap-2 sm:flex-row"><label className="sr-only" htmlFor="workspace-search">Search all accessible Quipsly records</label><input id="workspace-search" name="q" type="search" minLength={2} maxLength={120} defaultValue={focusedTag ? "" : query} placeholder="Try a task, client, episode, note, tag, or exact phrase" className="min-h-12 flex-1 rounded-xl border-2 border-[#d9c7a5] bg-white px-4 text-base font-semibold outline-none focus:border-sky-600 focus:ring-4 focus:ring-sky-100" /><button type="submit" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#3e2f21] px-6 text-xs font-black uppercase tracking-wide text-white"><Search size={17} aria-hidden="true" />Search</button></form><p className="mt-3 text-xs font-bold text-[#806a4d]" role="status">{!shouldSearch ? "Enter at least two characters. Nothing has been searched yet." : exactTagId && !focusedTag ? "That exact tag is not available in your accessible Nests. No record identities were disclosed." : `${resultCount} accessible result${resultCount === 1 ? "" : "s"}. Searched ${result.projectCount} accessible Nest${result.projectCount === 1 ? "" : "s"}.`}</p></header>
      {shouldSearch && (!exactTagId || focusedTag) ? <div className="grid gap-4 lg:grid-cols-2">
        ...
        <ResultSection title="Media clips" icon={Film}>{result.mediaClips.length && focusedTag ? <ul className="space-y-2">{result.mediaClips.map((item) => <li key={item.id}><Link href={mediaClipHref(item, focusedTag.resolvedTagId)} className="block rounded-xl border border-indigo-100 bg-indigo-50/40 p-3 hover:border-indigo-300"><strong>{item.title}</strong>{item.description ? <span className="mt-1 line-clamp-2 block text-sm font-semibold text-[#5e4a32]">{item.description}</span> : null}<span className="mt-1 block text-xs font-bold text-[#806a4d]">{item.mediaAsset.filename} · {item.inTimecode.toFixed(2)}s–{item.outTimecode.toFixed(2)}s{item.mediaAsset.isGlobal ? " · shared reference" : ""}</span></Link></li>)}</ul> : <Empty />}</ResultSection>
        <ResultSection title="Tags" icon={Tags}>{result.tags.length ? <ul className="space-y-2">{result.tags.map((item) => <li key={item.id}><Link href={tagFocusHref(item.id)} className="block rounded-xl border border-fuchsia-100 bg-fuchsia-50/40 p-3 hover:border-fuchsia-300"><strong>{item.label}</strong><span className="mt-1 block text-xs font-bold text-[#806a4d]">{item.project.name} · {String(item.category).replaceAll("_", " ")}{item.isPrivate ? " · private taxonomy" : ""}</span>{item.aliases.length ? <span className="mt-1 block text-xs font-semibold text-fuchsia-900">Former names: {item.aliases.map((alias) => alias.label).join(", ")}</span> : null}{item.description ? <span className="mt-1 line-clamp-2 block text-xs font-semibold text-[#765f40]">{item.description}</span> : null}</Link></li>)}</ul> : <Empty />}</ResultSection>
      </div> : null}
