...
describe("Search All page", () => {
  ...
  it("opens canonical task, goal, Session, document, source, and annotation results", async () => {
    ...
    jest.mocked(getPrismaClient).mockReturnValue({
      ...
      studioDocument: { findMany: jest.fn().mockResolvedValue([{ id: "document-1", title: "Episode outline", sourceLabel: "document-kind:note", projectionStatus: "private", project: { name: "High Ground", slug: "high-ground" }, blocks: [{ id: "block-1", title: null, body: "The opening needs a human proof-listen." }] }]) },
    ...
    expect(screen.getByRole("link", { name: "Episode outline The opening needs a human proof-listen. High Ground · note · private" })).toHaveAttribute("href", "/create?project=high-ground&document=document-1&block=block-1");
