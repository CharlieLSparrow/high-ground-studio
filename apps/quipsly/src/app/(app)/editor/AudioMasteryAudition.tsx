...
export type AudioMasteryProcessingDeltaPoint = {
  timeSeconds: number;
  sourceShortTermLufs: number;
  masteredShortTermLufs: number;
  deliveryDeltaLu: number;
  shapeDeltaLu: number;
};

export type AudioMasteryProcessingViewMode = "whole" | "minute" | "detail";

export type AudioMasteryProcessingViewSpan = {
  startSeconds: number;
  endSeconds: number;
  durationSeconds: number;
};
...
export function audioMasteryProcessingViewSpan(
  durationSeconds: number,
  selectedSeconds: number,
  mode: AudioMasteryProcessingViewMode,
...
export function audioMasteryProcessingDeltaSeries(
  source: AudioMasteryMeasurement,
  mastered: AudioMasteryMeasurement,
): AudioMasteryProcessingDeltaPoint[] {
  const toleranceMs = Math.max(source.seriesResolutionMs, mastered.seriesResolutionMs);
  const programShiftLu = mastered.integratedLufs - source.integratedLufs;
  return source.series.flatMap((sourcePoint) => {
    ...
    const deliveryDeltaLu = masteredPoint.shortTermLufs - sourcePoint.shortTermLufs;
    return [{
      timeSeconds: sourcePoint.timeMs / 1_000,
      sourceShortTermLufs: sourcePoint.shortTermLufs,
      masteredShortTermLufs: masteredPoint.shortTermLufs,
      deliveryDeltaLu,
      shapeDeltaLu: deliveryDeltaLu - programShiftLu,
    }];
  });
}

export function audioMasteryProcessingPointAt(
  points: AudioMasteryProcessingDeltaPoint[],
  seconds: number,
) {
  return points.reduce<AudioMasteryProcessingDeltaPoint | null>((nearest, point) => {
    if (!nearest) return point;
    return Math.abs(point.timeSeconds - seconds) < Math.abs(nearest.timeSeconds - seconds) ? point : nearest;
  }, null);
}

export function audioMasteryProcessingSummary(points: AudioMasteryProcessingDeltaPoint[]) {
  if (points.length === 0) {
...
function AudioMasteryProcessingChangeMap({
  source,
  mastered,
  diagnosis,
  selectedSeconds,
  ...
  const [viewMode, setViewMode] = useState<AudioMasteryProcessingViewMode>("whole");
  const titleId = useId();
  ...
  const span = audioMasteryProcessingViewSpan(duration, selectedSeconds, viewMode);
  const points = useMemo(() => audioMasteryProcessingDeltaSeries(source, mastered), [mastered, source]);
  const summary = useMemo(() => audioMasteryProcessingSummary(points), [points]);
  const selectedPoint = useMemo(() => audioMasteryProcessingPointAt(points, selectedSeconds), [points, selectedSeconds]);
  const visiblePoints = points.filter((point) => point.timeSeconds >= span.startSeconds && point.timeSeconds <= span.endSeconds);
  const visibleObservations = diagnosis?.observations.filter((observation) => observation.startSeconds <= span.endSeconds && observation.endSeconds >= span.startSeconds) ?? [];
  ...
  return <section className="mt-3 rounded-lg border border-sky-800 bg-[#111827] p-3" aria-label="Mastering processing change map">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div>
        <div className="text-xs font-black text-white">Processing change map</div>
      ...
      <div className="grid grid-cols-3 gap-1 rounded-lg border border-slate-700 bg-slate-900 p-1" role="group" aria-label="Processing change map zoom">
    ...
    <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[9px] font-black uppercase tracking-wide text-slate-300" aria-label="Processing change legend">
    ...
    <button type="button" onClick={selectFromMap} className="mt-3 block w-full overflow-hidden rounded-lg border border-slate-700 bg-slate-950 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-300" aria-label={`Processing change map from ${clock(span.startSeconds)} to ${clock(span.endSeconds)}. Select a position to move synchronized audition playback.`}>
      <svg viewBox={`0 0 ${width} ${height}`} className="h-44 w-full" role="img" aria-labelledby={`${titleId} ${descriptionId}`} preserveAspectRatio="none">
        <title id={titleId}>Mastering dynamic-shape change over the source clock</title>
        <desc id={descriptionId}>Bars above zero are relatively louder in the mastered preview after the uniform program shift is removed. Bars below zero are relatively quieter. Amber markers are deterministic source-signal observations. Cyan marks the synchronized audition playhead.</desc>
        ...
        {visiblePoints.map((point, index) => {
          ...
          return <rect key={`${point.timeSeconds}-${index}`} x={pointX} y={Math.min(center, pointY)} width={pointWidth} height={Math.max(1, Math.abs(center - pointY))} fill={point.shapeDeltaLu >= 0 ? "#34d399" : "#8b5cf6"} opacity="0.82"><title>{clock(point.timeSeconds)} delivery {point.deliveryDeltaLu >= 0 ? "+" : ""}{point.deliveryDeltaLu.toFixed(1)} LU; shape {point.shapeDeltaLu >= 0 ? "+" : ""}{point.shapeDeltaLu.toFixed(1)} LU</title></rect>;
        })}
    ...
    <div className="mt-3 grid grid-cols-2 gap-2 text-[9px] font-bold sm:grid-cols-5">
      ...
      <div className="rounded-lg bg-slate-900 p-2"><div className="font-mono text-sm font-black text-sky-200">{selectedPoint ? `${selectedPoint.deliveryDeltaLu >= 0 ? "+" : ""}${selectedPoint.deliveryDeltaLu.toFixed(1)}` : "—"}</div><div className="text-slate-400">Delivery delta LU</div></div>
    ...
    <p className="mt-3 text-[9px] font-bold leading-4 text-slate-400">Delivery delta includes overall level change. Shape delta subtracts the integrated program shift to expose relative dynamics. Neither value identifies a specific processor or certifies sound quality; use the map to seek, then compare at matched loudness.</p>
  </section>;
}
...
export function AudioMasteryAudition({
  ...
  return (
    ...
    <section className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-white sm:p-5" aria-label="Audio mastering audition desk">
      ...
      <AudioMasteryProcessingChangeMap source={source} mastered={mastered} diagnosis={diagnosis} selectedSeconds={currentTime} onSelect={seek} />
      {moments.length > 0 && (
        <div className="mt-3">
