...
export function NoteEditor({ initial, actorId }: { initial: Note; actorId: string }) {
  ...
  return <article className="mx-auto max-w-4xl text-quipsly-ink">
    ...
    <div className="rounded-2xl border border-quipsly-divider bg-quipsly-surface p-5 shadow-sm sm:p-9">
      ...
      {draft.blocks.map((block, index) => <textarea key={block.id} aria-label={draft.blocks.length === 1 ? "Note text" : `Note text ${index + 1}`} readOnly={!initial.canEditContent || !ready} maxLength={20_000} value={block.body} placeholder="Start writing…" rows={Math.max(14, block.body.split("\n").length + 2)} onChange={(event) => update({ ...draftRef.current, blocks: draftRef.current.blocks.map((item) => item.id === block.id ? { ...item, body: event.target.value } : item) })} onBlur={() => void persist()} className="block min-h-64 w-full resize-y border-0 bg-transparent text-base leading-8 outline-none placeholder:text-quipsly-muted" />)}
    </div>
  </article>;
}
