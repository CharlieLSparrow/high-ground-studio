...
export default async function ContinueCaptureWritingPage({ params }: { params: Promise<{ draftId: string }> }) {
  ...
  return <main className="grid min-h-screen place-items-center bg-[radial-gradient(circle_at_top,_#d5eee9,_transparent_44%),radial-gradient(circle_at_bottom_left,_#e1efd5,_transparent_38%),linear-gradient(180deg,#f3ead8,#e6dcc7)] px-5 py-10 text-quipsly-ink">
    <section className="w-full max-w-xl rounded-[2rem] border border-quipsly-divider bg-quipsly-surface/92 p-7 text-center shadow-xl shadow-[#5f4720]/10 sm:p-10">
      ...
      <a href={`quipsly://writing/${encodedDraftID}?action=continue`} className="mt-7 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-gradient-to-r from-quipsly-peacock-700 to-quipsly-fern-600 px-6 py-3 text-sm font-black text-[#fff7e8]"><Mic2 className="h-4 w-4" aria-hidden="true" />Continue in Quipsly Capture</a>
