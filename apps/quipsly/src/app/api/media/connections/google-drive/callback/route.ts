...
function redirectResult(request: Request, returnTo: string, state: string) {
  const target = googleDriveOAuthRedirectUrl({
...
export async function GET(request: Request) {
  ...
  if (!session?.user?.id)
    return redirectResult(request, "/projects", "signed-out");
  const url = new URL(request.url);
  if (url.searchParams.get("error"))
    return redirectResult(request, "/projects", "permission-denied");
  ...
  if (!code || !state || !cookieValue)
    return redirectResult(request, "/projects", "expired");
  ...
  try {
    ...
    await saveGoogleDriveConnection({
      ...
      encryptedRefreshToken: encryptGoogleDriveRefreshToken(
        token.refreshToken,
    ...
    console.error("[google-drive-oauth] callback failed", {
      code:
        error instanceof GoogleDriveOAuthError ? error.code : "callback-failed",
    });
