...
async function derivativeResponse(request: Request, id: string, headOnly = false) {
  ...
  const access = await resolveStudioProjectAccess({ projectSlug: derivative.project.slug, email, action: "read", prisma });
  if (!access.allowed || access.projectId !== derivative.projectId) return notFound();
  if (derivative.storageProvider !== "local" || process.env.NODE_ENV === "production") return notFound();

  const configuredRoot = path.resolve(
    process.env.QUIPSLY_LOCAL_MEDIA_UPLOAD_ROOT || path.join(tmpdir(), "quipsly-media-ingest"),
  );
  const [root, candidate] = await Promise.all([
    realpath(configuredRoot).catch(() => ""),
    realpath(derivative.locator).catch(() => ""),
  ]);
  if (!root || !candidate || !pathInside(root, candidate)) return notFound();
  const details = await stat(candidate).catch(() => null);
  if (!details?.isFile() || details.size !== Number(derivative.sizeBytes)) return notFound();
  ...
  if (range) {
    ...
    const body = headOnly ? null : Readable.toWeb(createReadStream(candidate, range)) as ReadableStream;
    return new Response(body, { status: 206, headers });
  }
  headers.set("Content-Length", String(details.size));
  const body = headOnly ? null : Readable.toWeb(createReadStream(candidate)) as ReadableStream;
