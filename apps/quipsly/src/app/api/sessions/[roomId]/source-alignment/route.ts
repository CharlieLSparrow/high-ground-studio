...
function handled(error: unknown) {
  ...
  return privateJson({
    ...
    error: "Quipsly could not verify this private alignment operation. Source timing and media were not changed.",
  }, 503);
}

export async function GET(request: Request, context: { params: Promise<{ roomId: string }> }) {
  const signedIn = await actor(request);
  if (!signedIn) return privateJson({ ok: false, code: "AUTH_REQUIRED", error: "Sign in before opening private Session sync evidence." }, 401);
  const roomId = text((await context.params).roomId);
  if (!roomId) return privateJson({ ok: false, code: "ROOM_REQUIRED", error: "Choose one Session before opening sync evidence." }, 400);
  try {
    return privateJson({ ok: true, ...await readSessionSourceAlignments({ prisma: getPrismaClient() as any, roomId, actor: signedIn }) });
  } catch (error) { return handled(error); }
}

export async function POST(request: Request, context: { params: Promise<{ roomId: string }> }) {
  const signedIn = await actor(request);
  if (!signedIn) return privateJson({ ok: false, code: "AUTH_REQUIRED", error: "Sign in before requesting private Session sync evidence." }, 401);
  const roomId = text((await context.params).roomId);
  let body: Record<string, unknown> = {};
  try { body = object(await request.json()); } catch { /* validated below */ }
  const action = text(body.action, 40).toUpperCase();
  if (!roomId) return privateJson({ ok: false, code: "ROOM_REQUIRED", error: "Choose one Session before requesting sync evidence." }, 400);
  try {
    ...
    return privateJson({ ok: false, code: "ACTION_UNSUPPORTED", error: "Choose queue or reconcile for Session sync evidence." }, 400);
  } catch (error) { return handled(error); }
}
