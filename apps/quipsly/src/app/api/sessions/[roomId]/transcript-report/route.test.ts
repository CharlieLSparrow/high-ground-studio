...
jest.mock("@/lib/prisma", () => ({
  getPrismaClient: jest.fn(() => ({
    recordingAsset: { findMany: mockRecordingAssetFindMany },
  })),
}));
...
jest.mock("@/lib/server/session-reviewed-source-placement", () => ({
  ...
  SessionReviewedSourcePlacementError: class SessionReviewedSourcePlacementError extends Error {
    code = "TRANSCRIPT_REVIEWED_PLACEMENT_INVALID";
  },
}));
    ...
    speakerGroups: [
      {
        ...
        attribution: { participantId: "coach" },
      },
      {
        providerSpeakerLabel: "Speaker 1",
        attribution: { participantId: "client" },
      },
    ],
    segments: [
      {
        ...
        speakerLabel: "Coach Example",
        speakerAttribution: { participantId: "coach" },
        acceptedVerification: { id: "verification-1" },
      },
      {
        id: "turn-2",
        startSeconds: 5,
        ...
        speakerLabel: "Client Example",
        speakerAttribution: { participantId: "client" },
...
describe("coaching transcript report route", () => {
  ...
  it("returns a private source-bound Word report for an accessible coaching Session", async () => {
    const response = await GET(
      new Request(
        "https://nest.quipsly.com/api/sessions/room-1/transcript-report",
      ),
    ...
    expect(response.headers.get("Content-Disposition")).toContain(
      "Certification Practice Transcript.docx",
    );
    expect(
  ...
  it("does not make a podcast room look like a coaching mentor report", async () => {
    ...
    expect(await response.json()).toEqual(
      expect.objectContaining({ code: "REPORT_COACHING_REQUIRED" }),
    );
  });
  ...
  it("assembles participant-isolated transcripts from the same coherent take", async () => {
    ...
    const isolated = (
      ...
      desk({
        ...
        sourceSha256:
          participantId === "coach" ? "a".repeat(64) : "b".repeat(64),
        processing: {
          routing: {
        ...
        recording: {
          id: recordingId,
          participantId,
          recordedStartedAt: startedAt.toISOString(),
        },
        speakerGroups: [],
        segments: [
          {
            id: segmentId,
            startSeconds: participantId === "coach" ? 0 : 2,
            endSeconds: participantId === "coach" ? 1 : 3,
            text,
            speakerLabel: label,
          },
        ],
    ...
    const response = await GET(
      new Request(
        "https://nest.quipsly.com/api/sessions/room-1/transcript-report?recordingAssetId=coach-source",
      ),
      { params: Promise.resolve({ roomId: "room-1" }) },
    );

    expect(response.status).toBe(200);
    expect(response.headers.get("X-Quipsly-Transcript-Schema")).toBe(
      "quipsly-coaching-transcript-report-v2",
    );
    expect(response.headers.get("X-Quipsly-Transcript-Source-Count")).toBe("2");
    expect(response.headers.get("X-Quipsly-Transcript-Timing")).toBe(
      "capture-clock-proposal",
    );
    expect(response.headers.get("X-Quipsly-Transcript-Waveform-Review")).toBe(
      "required",
    );
