...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, ...await readDialogueRepairStatus({ prisma, projectSlug: input.projectSlug, assetId: input.assetId, sourceId: input.sourceId }) }, { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const sourceCoordinates = { projectSlug: input.projectSlug, assetId: input.assetId, sourceId: input.sourceId };
    ...
    if (action === "queue-experiment") {
      const result = await queueDialogueRepairExperiment({ prisma, ...sourceCoordinates, actor, candidateId: text(body.candidateId) });
    ...
    const result = await appendDialogueRepairReview({ prisma, ...sourceCoordinates, actor, candidateId: text(body.candidateId), clientRequestId: text(body.clientRequestId), decision, playbackEvidence: body.playbackEvidence, note: typeof body.note === "string" ? body.note : null });
