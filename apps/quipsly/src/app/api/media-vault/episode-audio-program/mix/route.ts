...
import {
  EpisodeAudioMixError,
  queueEpisodeAudioMix,
  queueEpisodeAudioMixWaveformReview,
  readEpisodeAudioMix,
  reconcileEpisodeAudioMix,
  reconcileEpisodeAudioMixWaveformReview,
} from "@/lib/server/episode-audio-mix";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const operations = ["queue", "reconcile", "queue-waveforms", "reconcile-waveforms"];
    if (!body || invalid(input) || !operations.includes(operation) || (operation.endsWith("waveforms") && !input.jobId)) return NextResponse.json({ ok: false, code: "EPISODE_AUDIO_MIX_REQUEST_INVALID", error: "A supported operation plus Nest and Episode identities is required; waveform review also requires the exact mix job." }, { status: 400 });
    const prisma = getPrismaClient();
    ...
    const status = operation === "queue"
      ? await queueEpisodeAudioMix({ prisma, ...input, actorEmail: access.actor.email })
      : operation === "reconcile"
        ? await reconcileEpisodeAudioMix({ prisma, ...input })
        : operation === "queue-waveforms"
          ? await queueEpisodeAudioMixWaveformReview({ prisma, ...input, actorEmail: access.actor.email })
          : await reconcileEpisodeAudioMixWaveformReview({ prisma, ...input });
