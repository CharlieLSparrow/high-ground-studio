...
function digestSession(
  session: any,
  sourceExitReadiness: SourceExitReadiness | null = null,
) {
  ...
  const attentionChecks = lifecycleChecks
    .filter((check: any) =>
      ["missing", "attention"].includes(text(check?.status).toLowerCase()),
    )
    .map((check: any) => ({
      id: text(check?.id),
  ...
  return {
    ...
    attentionChecks,
    nextAction:
      session.actionPacket?.nextAction ||
      session.nextAction ||
      session.afterCaptureNextAction,
  };
}

function finishAction(session: ReturnType<typeof digestSession>) {
  if (!text(session.callRoomId) || Number(session.recordingCount || 0) <= 0)
    return null;
  ...
  if (exit && !exit.safeToLeaveAllEndpoints) {
    const endpointConfirmationOnly =
      exit.state === "SERVER_COPY_COMPLETE_DEVICE_CONFIRMATION_REQUIRED";
    return {
      callRoomId: session.callRoomId,
      title: session.title,
      purpose: session.purpose,
      stage: session.stage,
      kind: endpointConfirmationOnly
        ? "confirm-endpoint-drain"
        : "protect-recording-sources",
      label: exit.label,
      detail: exit.detail,
  ...
  if (!hasCanonicalMedia && capabilities.canPromoteRecordingToMedia === true) {
    return {
      ...
      detail:
        "The retained source is verified and eligible for explicit promotion into the canonical media inventory.",
      priority: 10,
    };
  }
  if (!hasCanonicalMedia && session.blockers.length > 0) {
    return {
      ...
      detail:
        session.nextAction ||
        `Review ${session.blockers[0]} before deriving or promoting this source.`,
      priority: 15,
    };
  }
  if (
    capabilities.canRunTranscript === true &&
  ...
  if (session.blockers.length > 0 || session.attentionChecks.length > 0) {
    return {
      ...
      detail:
        session.nextAction ||
        session.attentionChecks[0]?.meaning ||
        `Review ${session.blockers[0]} before continuing.`,
      priority: 50,
    };
  }
  return null;
...
function buildDigest(
  sessions: any[],
  sourceExitReadinessByRoom: ReadonlyMap<
    string,
    SourceExitReadiness
  > = new Map(),
) {
  ...
  const allFinishActions = sessionDigests
    ...
    .sort((left: any, right: any) => {
      ...
      const leftAttention =
        Date.parse(
          text(
            leftSession?.sourceExitReadiness?.attentionAt ||
              leftSession?.updatedAt ||
              leftSession?.scheduledStart,
          ),
        ) || 0;
      const rightAttention =
        Date.parse(
          text(
            rightSession?.sourceExitReadiness?.attentionAt ||
              rightSession?.updatedAt ||
              rightSession?.scheduledStart,
          ),
        ) || 0;
      return (
        rightAttention - leftAttention ||
  ...
  return {
    ...
    readyToCapture: countWhere(sessions, (session) =>
      bool(session.canRecordNow),
    ),
    needsConsent: countWhere(
      sessions,
      (session) =>
        sessionStage(session) === "consent-needed" ||
        !bool(session.recordingConsentGranted),
    ),
    paymentHold: countWhere(
      sessions,
      (session) =>
        sessionStage(session) === "payment-needed" ||
        session.captureReadiness?.status === "payment-hold",
    ),
    providerJoinReady: countWhere(sessions, (session) =>
      bool(session.providerCanJoin),
    ),
    localFallbackReady: countWhere(
      sessions,
      (session) => session.captureReadiness?.safeToRecordLocally === true,
    ),
    recordingEvidence: countWhere(
      sessions,
    ...
    recordingPromotionReady: countWhere(
      sessions,
      (session) =>
        session.actionPacket?.capabilities?.canPromoteRecordingToMedia === true,
    ),
    recordingPromotedToMedia: countWhere(sessions, (session) =>
      Boolean(session.latestRecordingMediaAssetId),
    ),
    joinableProviderRooms: countWhere(
      sessions,
      (session) => session.actionPacket?.capabilities?.canJoin === true,
    ),
    locallyRecordableRooms: countWhere(
      sessions,
      (session) =>
        session.actionPacket?.capabilities?.canStartLocalRecording === true,
    ),
    transcriptRunnableRooms: countWhere(
      sessions,
      (session) =>
        session.actionPacket?.capabilities?.canRunTranscript === true,
    ),
    packetBuildableRooms: countWhere(
      sessions,
      (session) => session.actionPacket?.capabilities?.canBuildPacket === true,
    ),
    transcriptNeeded: countWhere(
      sessions,
      (session) =>
        sessionStage(session) === "transcription-needed" ||
        session.coachingPacketStatus === "NOT_READY",
    ),
    ...
    reviewReady: countWhere(
      sessions,
      (session) => session.lifecycle?.readyForReview === true,
    ),
    recoveryOpen: countWhere(
      sessionDigests,
      (session) =>
        Number(session.recordingCount || 0) > 0 &&
        Boolean(
          session.sourceExitReadiness &&
          !session.sourceExitReadiness.safeToLeaveAllEndpoints,
        ),
    ),
    safeToLeave: countWhere(
      sessionDigests,
      (session) =>
        Number(session.recordingCount || 0) > 0 &&
...
function sourceExitReadinessForRoom(
  ...
  for (const receipt of asArray(room.stateReceipts)) {
    ...
    if (!captureId || row.outcome !== "APPLIED" || row.stateApplied !== true)
      continue;
    ...
    current.status =
      current.startedAt && current.stoppedAt
  ...
  const topology = buildSessionReadinessTopology({
    participants: asArray(room.participants)
      .map((participant) => {
  ...
  const attentionAt = [
    room.updatedAt,
    ...asArray(room.endpointQueueReceipts).map(
      (receipt) => (receipt as any).reconciledAt || (receipt as any).createdAt,
    ),
    ...asArray(room.expectedSources).map(
      (source) => (source as any).updatedAt || (source as any).createdAt,
    ),
    ...asArray(room.recordingAssets).map(
      (asset) => (asset as any).updatedAt || (asset as any).createdAt,
    ),
    ...asArray(room.stateReceipts).map(
      (receipt) => (receipt as any).receivedAt || (receipt as any).createdAt,
    ),
    ...finalizationReceipts.map(
      (receipt) => receipt.updatedAt || receipt.createdAt,
    ),
  ]
  ...
  return {
    ...
    attentionAt: Number.isFinite(attentionAt)
      ? new Date(attentionAt).toISOString()
      : null,
    missingPlannedSources: topology.expectedSources
      ...
      .map((source) => ({
        ...
        retentionRole: source.retentionRole,
    ...
    resolvedCaptureEvidence: topology.people
      .flatMap((person) => person.sources)
      .concat(topology.unassignedSources)
      .filter((source) => source.serverRetention.state === "CAPTURE_PLAN_RESOLVED")
      .map((source) => ({
        id: source.id,
        label: source.label,
        captureId: source.captureId,
        status: source.status,
        disposition: source.planDisposition,
      })),
    endpointQueues: topology.people.flatMap((person) =>
      person.endpointQueues.map((queue) => ({
        id: queue.id,
...
export async function GET(request: Request) {
          ...
          ? [
              {
                project: {
                  accessGrants: {
                    some: { email: actorEmail, status: "ACTIVE" },
                  },
                },
              },
            ]
  ...
  const recordingAssetIds = rooms.flatMap((room: any) =>
    room.recordingAssets.map((asset: any) => asset.id),
  );
  const finalizationReceipts = recordingAssetIds.length
    ...
    : [];
  const sessions = mapMobileCaptureSessionsForUser({
    rooms,
    userId,
  ...
  for (const room of rooms as any[]) {
    const roomAssetIds = new Set(
      asArray(room.recordingAssets)
        .map((asset) => text((asset as any).id))
        .filter(Boolean),
    );
    const roomFinalizations = finalizationReceipts.filter(
      (receipt: any) =>
        receipt.roomId === room.id ||
        roomAssetIds.has(text(receipt.recordingAssetId)),
    );
