...
describe("explicit transcript-derived goal", () => {
  ...
  it("creates one actor-owned active goal with exact source evidence and no implied work", async () => {
    ...
    const created = { id: "goal-created", ownerUserId: "user-1", roomId: "room-1", title: "Publish the pilot", description: "A clear first episode.", status: "ACTIVE", createdAt: new Date("2026-07-18T22:00:00Z") };
    const tx = { goal: { findUnique: jest.fn().mockResolvedValue(null), create: jest.fn().mockResolvedValue(created) } };
    jest.mocked(getPrismaClient).mockReturnValue({ $transaction: jest.fn((callback: any) => callback(tx)) } as any);
    const response = await POST(request({ roomId: "room-1", segmentId: "segment-1", clientRequestId: "goal-request-1", expectedProviderTextSha256: "a".repeat(64), title: "Publish the pilot", description: "A clear first episode.", surface: "ios-capture" }));
    expect(response.status).toBe(200);
    expect(await response.json()).toMatchObject({ ok: true, goal: { ownerUserId: "user-1", status: "ACTIVE" }, boundaries: { taskCreated: false, targetDateCreated: false, calendarMutated: false, externalDelivery: false, publication: false } });
    expect(tx.goal.create).toHaveBeenCalledWith({ data: expect.objectContaining({ ownerUserId: "user-1", roomId: "room-1", projectId: "project-1", sourceJson: expect.objectContaining({ schema: "quipsly-transcript-derived-goal-v1", segmentId: "segment-1", providerTextSha256: "a".repeat(64), recordingAssetId: "asset-1", explicitHumanAction: true }) }) });
  });
