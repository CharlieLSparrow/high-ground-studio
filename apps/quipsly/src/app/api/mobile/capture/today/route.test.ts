...
const goalMergeGovernance = {
  schema: "quipsly-governed-action-reference-v1",
  runId: "run-goal-merge",
  actionId: "action-goal-merge",
  attemptId: "attempt-goal-merge",
  receiptId: "receipt-goal-merge",
  capabilityId: "quipsly.session.transcript-goal-evidence.merge",
  capabilityVersion: 1,
};
const taskMergeGovernance = {
  schema: "quipsly-governed-action-reference-v1",
  runId: "run-task-merge",
  actionId: "action-task-merge",
  attemptId: "attempt-task-merge",
  receiptId: "receipt-task-merge",
  ...
  it("returns canonical actor work while quarantining transcript candidates", async () => {
    ...
    jest.mocked(getPrismaClient).mockReturnValue({
      ...
      goal: { findMany: jest.fn().mockResolvedValue([
        { id: "goal-1", title: "Ship a trustworthy episode", description: null, status: "ACTIVE", targetAt: null, updatedAt: expected, sourceJson: { schema: "quipsly-transcript-derived-goal-v1", roomId: "room-1", transcriptJobId: "job-1", segmentId: "segment-1", startSeconds: 3.66, endSeconds: 4.84, providerTextSha256: "a".repeat(64), providerSpeakerLabel: "Speaker", effectiveTextSnapshot: "Welcome, everybody.", effectiveSpeakerLabelSnapshot: "Charlie", acceptedCorrectionId: null, recordingAssetId: "asset-1", playbackSourceId: "source-1" }, project: { id: "project-1", name: "High Ground", slug: "high-ground" }, tagLinks: [{ tag: { id: "tag-2", label: "Episode", slug: "episode", projectId: "project-1", isActive: true } }], room: { id: "room-1", title: "Episode review" }, progressReceipts: [{ id: "merge-progress-1", kind: "TRANSCRIPT_CANDIDATE_MERGED", progressPercent: null, note: "Reviewed transcript evidence.", occurredAt: persisted, evidenceJson: { schema: "quipsly-transcript-goal-evidence-merge-v1", receiptId: "review-receipt-1", goalCandidateId: "packet-goal-build-1-segment-1", mergedAt: "2026-07-18T18:00:00.000Z", candidateSource: { schema: "quipsly-transcript-derived-goal-v1", roomId: "room-1", transcriptJobId: "job-1", segmentId: "segment-1", startSeconds: 3.66, endSeconds: 4.84, providerTextSha256: "a".repeat(64), providerSpeakerLabel: "Speaker", effectiveTextSnapshot: "Welcome, everybody.", effectiveSpeakerLabelSnapshot: "Charlie", acceptedCorrectionId: null, recordingAssetId: "asset-1", playbackSourceId: "source-1" } } }, { id: "numeric-progress-1", kind: "MANUAL_CHECK_IN", progressPercent: 35, note: "One proof pass complete.", occurredAt: expected, evidenceJson: null }] },
    ...
    expect(payload.tasks[1]).toMatchObject({ id: "task-2", todayReason: "From session transcript" });
    ...
    expect(payload.tasks[0]).toMatchObject({ id: "task-1", todayReason: "Planned focus · session transcript", canEditTags: true, tagIds: ["tag-1", "tag-archived"], tagLabels: ["Proof listen", "Legacy review"], sourceAnchor: { roomId: "room-1", segmentId: "segment-1", startSeconds: 3.66, recordingAssetId: "asset-1" }, lastMergedTranscriptEvidence: { receiptId: "task-review-receipt-1", governance: { actionId: "action-task-merge", receiptId: "receipt-task-merge", capabilityId: "quipsly.session.transcript-task-evidence.merge" } } });
