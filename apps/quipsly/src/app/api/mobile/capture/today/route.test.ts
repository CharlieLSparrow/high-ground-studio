...
describe("mobile Capture Today contract", () => {
  ...
  it("returns canonical actor work while quarantining transcript candidates", async () => {
    ...
    expect(payload.tasks[0]).toMatchObject({ id: "task-1", todayReason: "Planned focus · reviewed transcript", sourceAnchor: { roomId: "room-1", segmentId: "segment-1", startSeconds: 3.66, recordingAssetId: "asset-1" } });
    ...
    expect(payload.goals[0]).toMatchObject({ id: "goal-1", roomId: "room-1", sessionTitle: "Episode review", sourceAnchor: { schema: "quipsly-transcript-derived-goal-v1", segmentId: "segment-1", startSeconds: 3.66, recordingAssetId: "asset-1" } });
