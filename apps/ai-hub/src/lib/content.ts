  ...
  try {
    ...
    yamlStr.split("\n").forEach((line) => {
      ...
      if (colIndex !== -1) {
        ...
        // Parse array notation if it starts with YAML sequence
        if (key === "tags" || key === "scopes") {
          // Handled as array if parsed from sequence
          return;
        }
...
export function markdownToHtml(markdown: string, accentColor: "teal" | "amber" | "rose" = "teal"): string {
  let html = markdown
  ...
  html = html.replace(/^# (.*$)/gim, '<h1 class="text-3xl font-extrabold mt-10 mb-6 text-slate-100">$1</h1>');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("<li")) {
      ...
      if (line.startsWith("<h") || line.startsWith("<blockquote") || line.startsWith("---")) {
        ...
        formattedLines.push(`<p class="leading-relaxed mb-4 text-slate-300">${line}</p>`);
      }
    }
  }
