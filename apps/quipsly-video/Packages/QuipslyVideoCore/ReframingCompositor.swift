...
public class ReframingCompositor: NSObject, AVVideoCompositing {
    ...
    private func interpolate(keyframes: [FramingKeyframe], timeSecs: Double) -> (Float, simd_float3x3) {
        ...
        let getFov = { (k: FramingKeyframe) -> Float in
            let f = (Float(k.scale) * 180.0) * (Float.pi / 180.0)
            return min(max(f, 0.001), Float.pi - 0.001)
        }
        let getYaw = { (k: FramingKeyframe) -> Float in ((Float(k.offsetX) - 0.5) * 360.0) * (Float.pi / 180.0) }
        let getPitch = { (k: FramingKeyframe) -> Float in ((Float(k.offsetY) - 0.5) * 180.0) * (Float.pi / 180.0) }
