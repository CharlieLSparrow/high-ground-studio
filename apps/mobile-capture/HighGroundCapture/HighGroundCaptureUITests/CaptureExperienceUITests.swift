...
final class CaptureExperienceUITests: XCTestCase {
    ...
    func testConfirmedRequestHasImmediateSessionHandoff() {
        ...
        XCTAssertTrue(
            app.tabBars.buttons["Sessions"].isSelected,
    ...
    private func exerciseStandardCallGreenRoom() {
        ...
        let deviceOptions = app.buttons["CaptureCallOpenDevices"]
        ...
        XCTAssertTrue(join.isHittable, "Join must be visible without scrolling through shared work or recording setup.")
        XCTAssertTrue(deviceOptions.exists, "Device settings remain available without becoming a required setup step.")
        let thisDevice = app.buttons["CaptureCallAudioThisDevice"]
        let anotherDevice = app.buttons["CaptureCallAudioOtherDevice"]
        XCTAssertTrue(thisDevice.exists && anotherDevice.exists, "The remembered call-audio destination must be visible before Join.")
        thisDevice.tap()
        ...
        turnOn(useCallAudio)
        XCTAssertTrue(outputRoute.exists, "Devices should show the actual listening route separately from the microphone.")
        XCTAssertTrue(routePicker.exists, "Devices should expose Apple's familiar system audio-route control.")
        ...
        app.buttons["Done"].tap()
        thisDevice.tap()
        XCTAssertEqual(microphone.label, "Microphone on", "Returning audio here must not require a settings detour.")
        anotherDevice.tap()
        XCTAssertEqual(microphone.label, "Microphone is on another device")
        thisDevice.tap()
        XCTAssertTrue(join.isHittable, "Changing audio destination must keep Join reachable.")
        XCTAssertFalse(routePicker.exists, "Device administration belongs in its sheet, not between the person and Join.")
    ...
    func testCallLobbyRemembersSafeDeviceChoicesAcrossRelaunch() {
        ...
        let restoredDeviceOptions = app.buttons["CaptureCallOpenDevices"]
    ...
    func testSourceEvidencePreviewShowsTruthBoundariesWithoutCreatingAReceipt() throws {
        ...
        let loudnessSummary = app.descendants(matching: .any)[
            "CaptureAudioLoudnessSummary"
        ]
        XCTAssertTrue(loudnessSummary.exists)
        XCTAssertTrue(loudnessSummary.label.contains("ITU-R BS.1770-5"))
...
final class CaptureAppStoreScreenshotUITests: XCTestCase {
    ...
    func testCapturePrivateDataSafeDrafts() {
        ...
        launch(tab: "record", waitingFor: app.navigationBars["Sessions"])
        let chooser = app.buttons["CaptureSessionChooser"]
        XCTAssertTrue(chooser.waitForExistence(timeout: 5))
        chooser.tap()
