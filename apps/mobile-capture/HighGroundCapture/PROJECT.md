...
| Local video | User-reachable Audio / Solo video / Podcast camera picker, real front/rear preview, actual-device 1080p–4K/30 profile resolution, HEVC preference, ten-second MOV fragments, separate audio/video consent readiness, controlled pause/camera-switch source boundaries, thermal/storage/foreground/account interlocks, full-track EOF validation, persisted finalized MOV facts with fail-closed profile-drift holds, typed upload evidence, and real Library video playback | Simulator build, source contracts, and operated mode UX pass; physical camera/watch, 4K/orientation/thermal/endurance, upload, proxy, and editor-alignment matrix pending |
