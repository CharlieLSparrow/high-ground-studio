...
| Owner Nest backup and preview-first restore | generic dual-architecture Simulator build, strict Nest TypeScript, 27-project pinned TypeScript 7.0.2 gate, 17/17 focused shared-contract/route/web tests, 1/1 disposable PostgreSQL graph round trip, 91-check mobile source contract, and focused iPhone 17 Pro / iOS 26.3.1 Simulator Account journey | PASS — Account route, owner destination disclosure, protected/versioned export contract, security-scoped import, read-only validation, transaction-level reviewed-plan precondition before writes, exact apply readback, web/native parity, exclusion boundary, deterministic retry, cleanup, separate confirmation, and accessibility audit; 1/1 UI at `/private/tmp/quipsly-nest-portability-final.z7tfy7/result.xcresult`; authenticated HTTP export/apply plus physical Files/Share Sheet handling and production remain open |
