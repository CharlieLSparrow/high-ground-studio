...
capture_phone_shell="$capture_root/HighGroundCapture/CapturePhoneShell.swift"
export_options="$capture_root/fastlane/ExportOptions.plist"
fastfile="$capture_root/fastlane/Fastfile"
artifact_verifier="$capture_root/scripts/verify-release-artifact.sh"
...
transcript_ledger_policy_test="$capture_root/scripts/test-on-device-transcript-ledger-policy.sh"
...
require_text "$app_info_plist" "Use your camera for video calls and video recordings you start." "Camera purpose string is concise and scoped to user-started video"
require_text "$app_info_plist" "Use your microphone for calls and audio recordings you start." "Microphone purpose string is concise and scoped to user-started audio"
require_text "$app_info_plist" "Turn your recordings into editable transcripts." "Speech purpose string is concise and outcome-oriented"
...
require_text "$artifact_verifier" '"UIDeviceFamily" '\''[1,2]'\''' "Packaged artifact verification requires native iPhone and iPad support"
require_text "$provider_room" "configuration.supportsVideo = true" "CallKit supports the user-controlled native video conversation"
require_text "$capture_phone_shell" "Recording starts only when someone taps Record." "CallKit video does not imply retained recording"
...
require_text "$fastfile" "reinstall_app: true" "Each local UI shard begins from a reinstalled app container"
require_text "$fastfile" "app_identifier: APP_IDENTIFIER" "App-container reinstall uses the exact shipping bundle identifier"
...
require_text "$isolated_release_runner" 'QUIPSLY_CAPTURE_XCTEST_BASELINE_PATH' "Committed release exports its protected XCTest-device baseline"
require_text "$fastfile" 'cleanup_capture_ui_xctest_devices' "Capture UI qualification removes only completed disposable XCTest clones between executions"
require_text "$fastfile" 'next if baseline.key?(device_id)' "Capture UI qualification preserves every pre-existing XCTest device"
require_text "$fastfile" 'next if File.symlink?(device_path)' "Capture UI qualification refuses symlinked XCTest cleanup targets"
...
require_text "$fastfile" 'lane :merge_ui_evidence do |options|' "Capture can merge independently retained exact-source UI shards after an interrupted qualification"
require_text "$fastfile" 'verify_ui_test_evidence_path(output_path, expected_source_revision: source_revision, require_full: true)' "Merged UI evidence must independently prove complete source-derived coverage"
...
require_text "$isolated_release_runner" 'ui_test | merge_ui_evidence | candidate | release | beta | recover_candidate | seal_candidate | upload_qualified' "Isolated release accepts resumable UI shards, artifact recovery, proof sealing, and sealed-candidate upload reuse"
require_text "$fastlane_runner" 'ui_test|merge_ui_evidence|candidate|release|beta|recover_candidate|seal_candidate|upload_qualified' "Pinned Capture runner accepts resumable UI shards, artifact recovery, proof sealing, and sealed-candidate upload reuse"
...
bash "$transcript_ledger_policy_test"
pass "On-device transcript ledger integrity policy passes"
