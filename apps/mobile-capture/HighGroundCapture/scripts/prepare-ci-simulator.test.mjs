...
for (const scenario of arch === "unsupported" ? ["invalid-architecture"] : ["success", "already-booted", "missing", "duplicate", "boot-failure", "safari-failure", "invalid-variable", "local"]) {
  test(`simulator startup hands its exact identity and ${arch} architecture to the test step: ${scenario}`, t => {
    ...
    writeFileSync(log, "");
    writeFileSync(path.join(fixture, "uname"), `#!/bin/sh\nprintf '%s\\n' '${arch}'\n`, { mode: 0o700 });
...
`, { mode: 0o700 });
    ...
    assert.equal(readFileSync(envFile, "utf8"), passes && scenario !== "local" ? `CAPTURE_DESTINATION=platform=iOS Simulator,id=${id},arch=${arch}\n` : "");
    if (["missing", "duplicate"].includes(scenario)) assert.match(result.stderr, /Expected one available/);
    if (["invalid-variable", "invalid-architecture"].includes(scenario)) assert.equal(readFileSync(log, "utf8"), "");
    if (passes) {
