#!/usr/bin/env python3
"""Index one-short cut-quality review packets.

Review packets merge playable media, visual evidence, audio/cadence evidence,
questions, and note commands for one short. This index keeps the latest packet
per short findable without recording approval, edits, exports, or receipts.
"""
...
DEFAULT_PACKET_ROOT = DEFAULT_ROOT / "shorts-command-room" / "cut-quality-review-packets"
DEFAULT_OUTPUT_DIR = DEFAULT_PACKET_ROOT / "index"
DEFAULT_BASENAME = "quipsly-studio-shorts-cut-quality-review-packet-index"
SCHEMA = "quipsly.studio.shorts-cut-quality-review-packet-index.v1"
...
def discover_packets(root: Path) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    if not root.exists():
        return rows
    for json_path in sorted(root.glob("*/*/*review-packet.json")):
        if "/index/" in str(json_path):
            continue
        payload = read_json(json_path)
        if payload.get("schema") != "quipsly.studio.shorts-cut-quality-review-packet.v1":
            continue
        ...
        rows.append(
            {
                ...
                "visualFrameCount": len(visual.get("frames") or []),
                ...
                "missingEvidence": payload.get("missingEvidence") or [],
                "noteCommandCount": len(payload.get("noteCommandTemplates") or {}),
...
def build_index(root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    ready_evidence = [
        ...
        if row.get("visualStatus") == "available" and row.get("audioStatus") == "available" and not row.get("missingEvidence")
    ]
    return {
        "schema": SCHEMA,
        "version": VERSION,
        ...
        "counts": {
            "reviewPackets": len(rows),
            "shortsWithReviewPackets": len(latest),
            "latestEvidenceComplete": len(ready_evidence),
            "latestMissingEvidence": len(latest) - len(ready_evidence),
            "latestCadenceReview": cadence.get("cadence-review", 0),
            "cadenceLabels": dict(sorted(cadence.items())),
            "episodes": dict(sorted(episodes.items())),
        },
        "latestByShort": latest,
        "allReviewPackets": sorted(rows, key=lambda row: str(row.get("generatedAt") or ""), reverse=True),
        "truth": (
            "Review-packet index only. It records no review decision, edits no timeline, exports no media, "
            "publishes nothing, uploads nothing, transcribes nothing, mutates no media, overwrites no packet, "
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Shorts cut-quality review packet index",
        "",
        payload.get("truth", ""),
        "",
        "## Counts",
        "",
        f"- Review packets: `{counts.get('reviewPackets', 0)}`",
        f"- Shorts with review packets: `{counts.get('shortsWithReviewPackets', 0)}`",
        f"- Latest evidence complete: `{counts.get('latestEvidenceComplete', 0)}`",
        f"- Latest cadence-review: `{counts.get('latestCadenceReview', 0)}`",
        "",
        "## Latest review packet per short",
        "",
    ]
    latest = payload.get("latestByShort") if isinstance(payload.get("latestByShort"), list) else []
    if not latest:
        lines.append("- No review packets found yet.")
    for row in latest:
        ...
        lines.extend(
            [
                ...
                f"- Visual/audio: `{row.get('visualStatus')}` / `{row.get('audioStatus')}`",
                f"- Cadence: `{row.get('cadenceLabel')}`",
                f"- Note commands: `{row.get('noteCommandCount')}`",
    ...
    for row in payload.get("latestByShort", []):
        ...
        cards.append(
            ...
            <article class="card">
              ...
              <div class="pills">
                <span>visual {esc(row.get('visualStatus'))}</span>
                <span>audio {esc(row.get('audioStatus'))}</span>
                <span>{esc(row.get('cadenceLabel'))}</span>
                <span>{esc(row.get('noteCommandCount'))} note commands</span>
              </div>
              <p>{esc(row.get('nextSafestAction'))}</p>
              <a class="button" href="{esc(file_uri(html_path))}">Open review packet</a>
...
<head>
  ...
  <title>Quipsly shorts review packet index</title>
  <style>
    :root {{ color-scheme: dark; --soil:#15110b; --moss:#1d3121; --leaf:#8edc89; --honey:#f3ce54; --cream:#fff1d4; --line:rgba(255,241,212,.16); }}
    * {{ box-sizing:border-box; }}
    body {{ margin:0; font-family:ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; color:var(--cream); background:radial-gradient(circle at 10% -8%,rgba(142,220,137,.24),transparent 28rem),linear-gradient(135deg,var(--moss),var(--soil)); }}
...
<main>
  <header>
    <p class="eyebrow">Quipsly Studio · review packet index</p>
    <h1>Latest one-short cockpits, ready to review.</h1>
    <p>{esc(payload.get('truth'))}</p>
    <div class="metrics">
      <div><strong>{esc(counts.get('reviewPackets', 0))}</strong><span>packets</span></div>
      <div><strong>{esc(counts.get('shortsWithReviewPackets', 0))}</strong><span>shorts covered</span></div>
      <div><strong>{esc(counts.get('latestEvidenceComplete', 0))}</strong><span>evidence complete</span></div>
      <div><strong>{esc(counts.get('latestCadenceReview', 0))}</strong><span>cadence review</span></div>
    </div>
  </header>
  <section class="grid">{''.join(cards) if cards else '<p>No review packets found yet.</p>'}</section>
</main>
</body>
</html>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Index cut-quality review packets.")
    parser.add_argument("--packet-root", default=str(DEFAULT_PACKET_ROOT), help="Root folder containing review packet artifacts.")
    ...
    payload = build_index(Path(args.packet_root).expanduser(), Path(args.output_dir).expanduser())
