...
SESSION_NAME="${QUIPSLY_SELECTED_SHORT_SESSION:-episode-1-premiere-rescue}"
OUTPUT_DIR="${QUIPSLY_SHORT_EXPORT_SMOKE_DIR:-/tmp/quipslystudio-short-export-smoke}"
NO_BUILD=0

usage() {
  cat <<'USAGE'
Smoke selected-short 9:16 export for any production-ready native session.

Usage:
  script/smoke_selected_short_export.sh [--no-build] [--session <name>] [--output <directory>]

This proves:
  - A production-ready native session can create a temporary 9:16 short recipe from an existing SHOW decision.
...
if [[ "$NO_BUILD" == "1" ]]; then
  ...
  "$ROOT_DIR/script/build_and_run.sh" --verify >/tmp/quipslystudio-selected-short-export-build.log
fi

"$ROOT_DIR/script/agentctl.sh" load-session "$SESSION_NAME" >/tmp/quipslystudio-selected-short-export-load.json
...
try:
    ...
    if state.get("productionReady") is not True:
        errors.append(f"Session should be production-ready for short export, got {state.get('productionReady')!r}: {state.get('productionReadinessDetail')}")
    ...
    if errors:
        print("Selected-short export smoke failed before export:", file=sys.stderr)
        for error in errors:
    ...
    basename = "codex-smoke-selected-short"
    command(
