#!/usr/bin/env python3
"""Quipsly Patreon publish worker.

Current contract:
- dry-run only
- reads one Quipsly publish worker payload JSON path from argv
- validates artifact, destination guidance, post copy, and receipt-contract shape
...
SUPPORTED_PLATFORMS = {"Patreon"}
SUPPORTED_DESTINATIONS = {"patreon_episode"}
...
def main() -> int:
    if len(sys.argv) != 2:
        return fail("Usage: patreon_upload_worker.py /absolute/path/to/payload.json")
    ...
    if platform not in SUPPORTED_PLATFORMS:
        return fail("This worker only supports Patreon.", platform=platform)
    ...
    if not title:
        return fail("Patreon post copy is missing a title.", artifactPath=artifact_path)
    if not description:
        return fail("Patreon post copy is missing body/description text.", artifactPath=artifact_path)
    ...
    result = {
        ...
        "wouldAttachArtifact": True,
        "networkCallsMade": False,
        "providerReceiptId": "",
        "publicURL": "",
        "receiptJson": {},
        "completedAt": datetime.now(timezone.utc).isoformat(),
        "safety": "Dry run only. No Patreon post, attachment, publication, provider receipt, or public URL was created.",
    }
