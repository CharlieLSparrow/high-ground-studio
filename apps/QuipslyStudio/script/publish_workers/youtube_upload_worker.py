#!/usr/bin/env python3
"""Quipsly YouTube publish worker.

Current contract:
- dry-run only
- reads one Quipsly publish worker payload JSON path from argv
- validates artifact, destination guidance, metadata, and receipt-contract shape
...
SUPPORTED_PLATFORMS = {"YouTube", "YouTube Shorts"}
SUPPORTED_DESTINATIONS = {"youtube_episode", "youtube_short"}
...
def main() -> int:
    if len(sys.argv) != 2:
        return fail("Usage: youtube_upload_worker.py /absolute/path/to/payload.json")
    ...
    if platform not in SUPPORTED_PLATFORMS:
        return fail("This worker only supports YouTube and YouTube Shorts.", platform=platform)
    ...
    if not title:
        return fail("YouTube upload copy is missing a title.", artifactPath=artifact_path)
    if not description:
        return fail("YouTube upload copy is missing a description.", artifactPath=artifact_path)
    ...
    result = {
        ...
        "safety": "Dry run only. No upload, scheduling, publishing, provider receipt, or public URL was created.",
    }
