...
NO_BUILD=false
OUTPUT_DIR="${TMPDIR:-/tmp}/quipslystudio-podcast-worker-smoke"
WORKER_PATH="$ROOT_DIR/script/publish_workers/podcast_upload_worker.py"

usage() {
  cat <<'USAGE'
Smoke Episode 1 podcast worker dry-run.

Usage:
  script/smoke_episode1_podcast_worker_dry_run.sh [--no-build] [--output-dir /absolute/output]

This proves the app can invoke the bundled podcast dry-run worker against both
Spotify and Apple Podcasts records without creating fake provider receipts or
public URLs.
USAGE
}
...
if [[ "$NO_BUILD" == false ]]; then
  "$ROOT_DIR/script/build_and_run.sh" --verify >/tmp/quipslystudio-podcast-worker-build.log
fi
...
if state.get("activeSessionName") != "episode-1-premiere-rescue" or state.get("productionReady") is not True:
    raise SystemExit("Episode 1 did not become production ready before podcast worker smoke.")
...
for platform in platforms:
    ...
    record = next((item for item in records if item.get("platform") == platform and item.get("deliveryLaneId") == "podcast-audio-master"), {})
    ...
    if "wouldCreateOrUpdateEpisode" not in (worker.get("resultJson") or ""):
        errors.append(f"{platform}: worker result JSON missing podcast dry-run intent: {worker}")
    ...
    if "wouldRequestDirectorySync" not in record.get("uploadJobJson", ""):
