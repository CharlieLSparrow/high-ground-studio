...
| Audio Intelligence | Source-bound waveform, spectrum, loudness, audible-event map, repair experiments, mastery review, delivery evidence, append-only human labels, immutable evidence-linked Episode mix proposals with exact-source rendering, and exact-context detector decisions in the shared Session queue | Observable and partially recoverable; one retained real-source baseline mix is independently verified | Loudness-matched baseline/proposal A/B, a representative two-person podcast/coaching corpus, qualified bleed/overlap/ducking automation, remaining authority-specific decisions in shared context, and destination-ready proof listening |
