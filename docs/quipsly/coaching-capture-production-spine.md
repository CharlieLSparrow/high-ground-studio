...
`GET /api/coaching/calendar/readiness` is the side-effect-free calendar readiness surface. Signed-in users can read the non-secret configured posture. Staff can add `?verify=1` to perform a read-only Google Calendar event-collection check using the same narrow scope and collection as managed event writes. The probe requests a partial response restricted to the collection kind, never requests event content, and returns only readiness state. It must not create, update, delete, send, invite, or schedule anything externally; it only proves whether Quipsly can reach the configured event collection before an operator syncs booking evidence.
