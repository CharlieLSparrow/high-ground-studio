 ...
 *
 * Transcript-derived work keeps source provenance so it can be understood,
 * edited, or removed. Provenance is not an approval gate.
 */
...
export const LEGACY_WEB_TRANSCRIPT_PACKET_SOURCE =
  "web-transcript-packet-builder" as const;
...
export type TranscriptSpeakerAuthority =
  (typeof TRANSCRIPT_SPEAKER_AUTHORITIES)[number];
...
const TRANSCRIPT_PACKET_BRIEF_SECTIONS = [
  ...
  },
  {
    ...
    pattern: /\?|\b(question|unclear|wonder|need to know|find out|verify)\b/i,
  },
] as const;
...
function briefSegment(segment: TranscriptPacketBriefSegment) {
  ...
  return {
    ...
    segmentIds:
      Array.isArray(segment.segmentIds) && segment.segmentIds.length
        ? [...segment.segmentIds]
        : [segment.id],
...
export function buildTranscriptPacketBrief(
  ...
  const section = (
    id: string,
    label: string,
    candidates: TranscriptPacketBriefSegment[],
  ) => {
    const seen = new Set<string>();
    const items = [];
    for (const segment of candidates) {
      if (!segment.id || seen.has(segment.id) || !briefText(segment.text))
        continue;
      seen.add(segment.id);
  ...
  const structuredSections = TRANSCRIPT_PACKET_BRIEF_SECTIONS.map(
    (definition) =>
      section(
        definition.id,
        definition.label,
  ...
  return {
    ...
    sourceTruth:
      "Every brief item points to an immutable transcript segment; recording media remains source truth.",
    overview: {
      segmentCount: segments.reduce(
        (count, segment) =>
          count +
          (Array.isArray(segment.segmentIds) && segment.segmentIds.length
      ...
      speakerCount: new Set(
        segments
          .map((segment) => briefText(segment.speakerLabel))
          .filter(Boolean),
      ).size,
      startSeconds:
        typeof segments[0]?.startSeconds === "number"
          ? segments[0].startSeconds
          : null,
      endSeconds:
        typeof segments.at(-1)?.endSeconds === "number"
          ? segments.at(-1)!.endSeconds
          : null,
    },
    sections: [
      ...structuredSections,
...
/** Build source-linked follow-through. The caller may materialize it directly. */
export function createTranscriptActionCandidate(input: {
  ...
  const committedActionItemId = input.committedActionItemId ?? null;
  return {
    ...input,
    kind: TRANSCRIPT_ACTION_CANDIDATE_KIND,
    reviewStatus: committedActionItemId
      ? "ACCEPTED_AS_ACTION_ITEM"
      : "READY_FOR_HUMAN_REVIEW",
    humanApprovalRequired: false,
    committedActionItemId,
  };
}
...
export function isTranscriptPacketSource(
  value: unknown,
): value is TranscriptPacketSource {
...
export function isTranscriptActionReviewDecision(
  ...
  return TRANSCRIPT_ACTION_REVIEW_DECISIONS.some(
...
export function isTranscriptGoalReviewDecision(
  ...
  return TRANSCRIPT_GOAL_REVIEW_DECISIONS.some(
    (decision) => decision === value,
  );
}
...
export function isTranscriptNoteReviewDecision(
  ...
  return TRANSCRIPT_NOTE_REVIEW_DECISIONS.some(
    (decision) => decision === value,
  );
}
...
export function transcriptPacketNoteCandidateId(
  packetBuildId: string,
  laneId: string,
  segmentId: string,
) {
...
export function isTranscriptActionCandidate(
  ...
  return (
    candidate.kind === TRANSCRIPT_ACTION_CANDIDATE_KIND &&
    ...
    (candidate.sourceText === undefined ||
      nonEmptyText(candidate.sourceText)) &&
    (candidate.sourceTextSha256 === undefined ||
      (typeof candidate.sourceTextSha256 === "string" &&
        /^[a-f0-9]{64}$/.test(candidate.sourceTextSha256))) &&
    (candidate.sourceSpan === undefined ||
      candidate.sourceSpan === null ||
      readTranscriptSourceSpan(candidate.sourceSpan) !== null) &&
    ...
    (candidate.speakerLabel === null ||
      typeof candidate.speakerLabel === "string") &&
    (candidate.speakerAuthority === undefined ||
      TRANSCRIPT_SPEAKER_AUTHORITIES.some(
        (authority) => authority === candidate.speakerAuthority,
      )) &&
 ...
 */
export function isUnreviewedTranscriptActionItemSource(
  value: unknown,
): boolean {
  const source = object(value);
  return (
...
export function selectLatestCorrelatedTranscriptPacketNotes<
  T extends TranscriptPacketNote,
>(notes: T[]) {
  const summaries = notes
    .filter((note) => note.kind === "SUMMARY")
    .sort((left, right) => {
      const createdDelta =
        packetTime(right.createdAt) - packetTime(left.createdAt);
      if (createdDelta !== 0) return createdDelta;
      const updatedDelta =
        packetTime(right.updatedAt) - packetTime(left.updatedAt);
  ...
  const highlights = packetBuildId
    ? allHighlights.filter(
        (note) =>
          packetText(object(note.sourceJson).packetBuildId) === packetBuildId,
      )
    : allHighlights;
...
export function readTranscriptActionCandidates(
  value: unknown,
): TranscriptActionCandidate[] {
  const source = object(value);
    ...
    return [
      {
        ...
        transcriptJobId:
          packetText(legacy.transcriptJobId) ||
          packetText(source.transcriptJobId),
        recordingAssetId:
          packetText(legacy.recordingAssetId) ||
          packetText(source.recordingAssetId),
        roomId: packetText(legacy.roomId) || packetText(source.roomId),
        packetBuildId:
          packetText(legacy.packetBuildId) || packetText(source.packetBuildId),
        segmentId: packetText(legacy.segmentId),
        speakerLabel: packetText(legacy.speakerLabel) || null,
        speakerAuthority: TRANSCRIPT_SPEAKER_AUTHORITIES.find(
          (authority) => authority === legacy.speakerAuthority,
        ),
        startSeconds:
          typeof legacy.startSeconds === "number" ? legacy.startSeconds : 0,
        endSeconds:
          typeof legacy.endSeconds === "number" ? legacy.endSeconds : 0,
        humanApprovalRequired:
          typeof legacy.humanApprovalRequired === "boolean"
...
function legacyTranscriptActionCandidate(
  item: LegacyTranscriptActionItem,
): TranscriptActionCandidate | null {
...
export function transcriptReleaseGate(
  ...
  if (heldReceipt) {
    return {
      ...
      evidenceKind: "HELD",
      errorCode:
        heldReceipt.transcriptDisposition !== "RELEASED"
          ? heldReceipt.transcriptHoldReasonCode ||
            "CAPTURE_TRANSCRIPT_EXPLICIT_RELEASE_REQUIRED"
          : heldReceipt.holdReasonCode ||
