...
function sessionHref(
  roomId: string,
  mode: "live" | "prepare" | "transcript" | "outputs",
) {
...
function isRecordingAttention(room: QuipslyCoachingPracticeRoom) {
  return (
    ["FAILED", "ERROR", "CORRUPT", "HELD"].includes(
      normalized(room.recordingStatus),
    ) ||
...
export function buildQuipslyCoachingPracticeCommand(
  ...
  if (now === null)
    throw new Error("A valid practice command clock is required.");
  ...
  for (const request of input.timeRequests) {
    ...
    if (
      normalized(request.status) !== "ACTIVE" ||
      expiresAt === null ||
      expiresAt <= now
    )
      continue;
    ...
    if (start < now && !["ENDED", "CANCELED"].includes(roomStatus)) {
      ...
      items.push(
        item({
          ...
          detail:
            "Its planned time has passed. Open it, complete it, or reschedule before it becomes hidden admin debt.",
  ...
  for (const room of input.rooms) {
    ...
    if (isRecordingAttention(room)) {
      items.push(
        item({
          ...
          detail:
            "Quipsly has recording evidence that needs review. The original remains the source of truth.",
    ...
    if (isTranscriptAttention(room)) {
      items.push(
        item({
          ...
          detail:
            "The transcript could not finish cleanly. Open its source-bound status and retry safely.",
    ...
    const transcriptCompleted =
      normalized(room.transcriptStatus) === "COMPLETED";
    ...
    if (ended && packetReady && room.followUpReleased !== true) {
      items.push(
        item({
          ...
          detail:
            "The packet is ready. Review the recording, transcript, notes, goals, and tasks before releasing it.",
    ...
    if (
      ...
      items.push(
        item({
          ...
          detail:
            "The transcript is ready for corrections, notes, goals, tasks, and a client-safe recording share.",
          actionLabel: "Review transcript",
          href: sessionHref(room.id, "transcript"),
          roomId: room.id,
          bookingId: room.bookingId ?? null,
  ...
  for (const booking of input.bookings) {
    if (!activeBookingStatuses.has(normalized(booking.status))) continue;
    const start = timestamp(booking.scheduledStart);
    if (start === null || start < now || start - now > preparationWindow)
      continue;
    if (booking.roomId && liveRoomIds.has(booking.roomId)) continue;
    const hasClientCheckIn =
      timestamp(booking.clientCheckInSubmittedAt) !== null;
  ...
  const futureBookings = input.bookings
    .filter((booking) => {
      const start = timestamp(booking.scheduledStart);
      return (
        activeBookingStatuses.has(normalized(booking.status)) &&
        start !== null &&
        start >= now
      );
    })
    .sort(
      (left, right) =>
        (timestamp(left.scheduledStart) ?? 0) -
          (timestamp(right.scheduledStart) ?? 0) ||
    ...
    items.push(
      item({
        ...
        title: nextBooking.clientLabel
          ...
          : nextBooking.title,
        detail:
          "Open the client space, invitation, Session plan, and device check from one place.",
  ...
  const deduped = [
    ...new Map(items.map((candidate) => [candidate.id, candidate])).values(),
  ].sort((left, right) => {
  ...
  const counts = {
    ...
    requests: deduped.filter(
      (candidate) => candidate.kind === "REVIEW_TIME_REQUEST",
    ).length,
    attention: deduped.filter((candidate) => candidate.tone === "attention")
      .length,
    prepare: deduped.filter((candidate) => candidate.kind === "PREPARE_SESSION")
      .length,
    followUp: deduped.filter((candidate) => candidate.tone === "follow-up")
      .length,
    today: deduped.filter((candidate) => isToday(candidate.scheduledAt, now))
      .length,
  };
