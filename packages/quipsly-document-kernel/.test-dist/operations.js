...
function finish(document, events) {
    document.updatedAt = nowIso();
    (0, anchors_1.refreshTextAnchorSnapshots)(document);
    return {
        document,
        projection: (0, projections_1.projectDocument)(document),
        events,
    };
}
function remapAnchorForInsert(anchor, nodeId, offset, length) {
    if (anchor.nodeId !== nodeId || length === 0)
        return anchor;
...
function remapAnchorForDelete(anchor, nodeId, startOffset, endOffset) {
    if (anchor.nodeId !== nodeId || startOffset === endOffset)
        return anchor;
    ...
    const nextStart = startOffset <= anchor.startOffset
        ? startOffset
        : anchor.startOffset;
...
function remapTextAnchors(document, mapper) {
    for (const annotation of document.annotations) {
        annotation.anchors = annotation.anchors.flatMap((anchor) => {
    ...
    for (const entityReference of document.entityReferences) {
        entityReference.anchors = entityReference.anchors.flatMap((anchor) => {
            if (!(0, anchors_1.isTextAnchor)(anchor))
                return [anchor];
            const mapped = mapper(anchor);
            return Array.isArray(mapped) ? mapped : [mapped];
...
function normalizeAnchorForDocument(document, anchor) {
    if (!(0, anchors_1.isTextAnchor)(anchor))
        return anchor;
    return (0, anchors_1.createTextAnchor)({
        document,
...
function applyOperation(sourceDocument, operation) {
    const document = (0, document_1.cloneDocument)(sourceDocument);
    const events = [];
    if (operation.type === "insertText") {
        const node = (0, document_1.getNodeOrThrow)(document, operation.nodeId);
        const offset = (0, anchors_1.normalizeTextRange)({
        ...
        remapTextAnchors(document, (anchor) => remapAnchorForInsert(anchor, operation.nodeId, offset, operation.text.length));
        events.push({
            type: "text-inserted",
    ...
    if (operation.type === "deleteText") {
        const node = (0, document_1.getNodeOrThrow)(document, operation.nodeId);
        const range = (0, anchors_1.normalizeTextRange)({
        ...
        remapTextAnchors(document, (anchor) => remapAnchorForDelete(anchor, operation.nodeId, range.startOffset, range.endOffset));
        events.push({
            type: "text-deleted",
    ...
    if (operation.type === "splitNode") {
        const nodeIndex = (0, document_1.getNodeIndex)(document, operation.nodeId);
        if (nodeIndex < 0)
            throw new Error(`Document node not found: ${operation.nodeId}`);
        ...
        const newNodeId = operation.newNodeId ?? (0, document_1.createId)("node");
        const nextNode = {
            ...node,
            id: newNodeId,
        ...
        if (offset === 0) {
            for (const boundary of document.boundaries) {
                if (boundary.nodeId === operation.nodeId)
                    boundary.nodeId = newNodeId;
            }
            for (const region of document.regions) {
                if (region.startNodeId === operation.nodeId)
                    region.startNodeId = newNodeId;
                if (region.endNodeId === operation.nodeId)
                    region.endNodeId = newNodeId;
            }
        }
        remapTextAnchors(document, (anchor) => splitTextAnchor({
            anchor,
            nodeId: operation.nodeId,
            newNodeId,
            offset,
    ...
    if (operation.type === "mergeNodes") {
        const leftIndex = (0, document_1.getNodeIndex)(document, operation.leftNodeId);
        const rightIndex = (0, document_1.getNodeIndex)(document, operation.rightNodeId);
        if (leftIndex < 0)
            throw new Error(`Document node not found: ${operation.leftNodeId}`);
        if (rightIndex < 0)
            throw new Error(`Document node not found: ${operation.rightNodeId}`);
        if (rightIndex !== leftIndex + 1) {
            throw new Error("mergeNodes requires adjacent left and right nodes.");
        }
        const hasBoundaryOnRight = document.boundaries.some((boundary) => boundary.nodeId === operation.rightNodeId);
        if (hasBoundaryOnRight) {
            throw new Error("Cannot merge across a boundary marker. Move or remove the boundary first.");
        }
        ...
        remapTextAnchors(document, (anchor) => {
            if (anchor.nodeId !== operation.rightNodeId)
                return anchor;
            return {
                ...anchor,
    ...
    if (operation.type === "addBoundary") {
        (0, document_1.getNodeOrThrow)(document, operation.nodeId);
        const boundary = {
            id: operation.id ?? (0, document_1.createId)("boundary"),
            kind: operation.kind,
            nodeId: operation.nodeId,
    ...
    if (operation.type === "removeBoundary") {
        document.boundaries = document.boundaries.filter((boundary) => boundary.id !== operation.boundaryId);
        events.push({
    ...
    if (operation.type === "addRegion") {
        (0, document_1.getNodeOrThrow)(document, operation.startNodeId);
        (0, document_1.getNodeOrThrow)(document, operation.endNodeId);
        const region = {
            id: operation.id ?? (0, document_1.createId)("region"),
            kind: operation.kind,
    ...
    if (operation.type === "applyInlineAnnotation") {
        const annotation = {
            id: operation.id ?? (0, document_1.createId)("annotation"),
            kind: operation.kind,
    ...
    if (operation.type === "removeAnnotation") {
        document.annotations = document.annotations.filter((annotation) => annotation.id !== operation.annotationId);
        events.push({
