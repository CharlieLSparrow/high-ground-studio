...
export class LandscapeIntroScene implements IScene {
  ...
  constructor(private engine: Engine) {
    ...
    this.timeline = createLandscapeIntroTimeline({
      camera: this.camera,
      sunMaterial: (sun as any).material,
      hazeObject: haze.object,
      titleMaterial: (title as any).material,
  ...
  update(dt: number, t: number) {
    ...
    // Entity updates
    for (const e of this.entities) e.update?.(t, dt);
