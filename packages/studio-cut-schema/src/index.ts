  ...
  return (
    ...
    (segment.words === undefined ||
      (Array.isArray(segment.words) &&
        segment.words.every(isTranscriptSegmentWord)))
  );
}

export function isTranscriptSegmentWord(
  value: unknown,
): value is TranscriptSegmentWord {
  if (!isRecord(value)) {
    return false;
  }

  const textKeys = ["text", "word", "token", "value", "w"] as const;
  const timeKeys = [
    "start",
    "startMs",
    "start_ms",
  ...
  const hasText = textKeys.some(
    (key) => typeof value[key] === "string" && value[key].trim().length > 0,
  );
  const validTimes = timeKeys.every((key) => {
